...
public class BugTrackerBugzilla extends BugTracker {
	...
	public void setDesc(Set<Alert> alerts) {
		...
		for(Alert alert: alerts) {
            ...
            if(alert.getName().length() > 0 ) {
                description.append(Constant.messages.getString("bugtracker.msg.alert") + alert.getName().toString() + "\n\n");
            }
            if(alert.getUri().length() > 0 ) {
                description.append(Constant.messages.getString("bugtracker.msg.url") + alert.getUri().toString() + "\n\n");
            }
            if(alert.getDescription().length() > 0 ) {
                description.append(Constant.messages.getString("bugtracker.msg.desc") + alert.getDescription().toString() + "\n\n");
            }
            if(alert.getOtherInfo().length() > 0 ) {
                description.append(Constant.messages.getString("bugtracker.msg.otherinfo") + alert.getOtherInfo().toString() + "\n\n");
            }
            if(alert.getSolution().length() > 0 ) {
                description.append(Constant.messages.getString("bugtracker.msg.solution") + alert.getSolution().toString() + "\n\n");
            }
            if(alert.getReference().length() > 0 ) {
                description.append(Constant.messages.getString("bugtracker.msg.reference") + alert.getReference().toString() + "\n\n");
            }
            if(alert.getParam().length() > 0 ) {
                description.append(Constant.messages.getString("bugtracker.msg.parameter") + alert.getParam().toString() + "\n\n");
            }
            if(alert.getAttack().length() > 0 ) {
                description.append(Constant.messages.getString("bugtracker.msg.attack") + alert.getAttack().toString() + "\n\n");
            }
            ...
            if(alert.getWascId() >= 0 ) {
                description.append(Constant.messages.getString("bugtracker.msg.wasc") + alert.getWascId() + ", ");
            }
