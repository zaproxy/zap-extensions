...
public class ViewStateDecoder {
	 ...
	 */
	public static Pattern patternNoHMAC = Pattern.compile("^\\s*\\<hmac\\>false\\</hmac\\>\\s*$", Pattern.CASE_INSENSITIVE | Pattern.MULTILINE);
	 ...
	 * @return a StringBuffer containing the human-readable and machine parseable representation (XML based)
	...
	/**
	 * decodes a Base64 encoded byte array into a human readable String, interpreting the 
	 * Base64 decoded data as a tree of ViewState objects.  
	 * @param base64encoded a byte array containing Base64 encoded ViewState data. 
	 * Should not contain superfluous characters at the end, as this breaks the "MAC" detection 
	...
	public String decodeAsXML (byte [] base64encoded) throws Exception{
		...
		if ( bytesremainingtoberead > 0 ) {
			...
			if (bytesremainingtoberead == 16)
				...
				representation.append("<hmactype>HMAC-UNKNOWN</hmactype>\n");
			representation.append(getIndentation(this.indentationlevel));
