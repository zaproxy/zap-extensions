...
public class StrictTransportSecurityScanner extends PluginPassiveScanner{
	...
	private static final Pattern BAD_MAX_AGE_PATT = Pattern.compile("\\bmax-age\\s*=\\s*\'*\"*\\s*0\\s*\"*\'*\\s*", Pattern.CASE_INSENSITIVE);
	//Ensure max-age actually contains a digit
	private static final Pattern MAX_AGE_PATT = Pattern.compile("\\bmax-age\\s*=\\s*\'*\"*\\s*\\s*\\d+\\s*\"*\'*\\s*", Pattern.CASE_INSENSITIVE);
	...
	private enum VulnType {HSTS_MISSING, HSTS_MAX_AGE_DISABLED, HSTS_MULTIPLE_HEADERS, HSTS_ON_PLAIN_RESP, HSTS_MAX_AGE_MISSING, HSTS_META, HSTS_MALFORMED_MAX_AGE, HSTS_MALFORMED_CONTENT};
		...
		if (msg.getRequestHeader().isSecure()){ //No point reporting missing for non-SSL resources
			...
			if (stsOption == null) { // Header NOT found
				...
				if (!wellformedMatcher.matches()) {
					//Well formed pattern didn't match (perhaps curly quotes or some other unwanted character(s))
	...
	/**
	 * Checks the source of the response for HSTS being set via a META tag which is explicitly
	 * not supported per the spec (rfc6797). 
	 * 
	 * @param source the source of the response to be analyzed.
	 * @return returns a string if HSTS was set via META (for use as alert evidence) otherwise return {@code null}.
		...
		if (metaElements != null) {
			for (Element metaElement : metaElements) {
				...
				if (STS_HEADER.equalsIgnoreCase(httpEquiv)) {
					return httpEquiv; //This is a META which attempts to define HSTS return it's value
				}
			}
		}
		return null;
