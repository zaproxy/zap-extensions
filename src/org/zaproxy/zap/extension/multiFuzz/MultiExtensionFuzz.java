...
public class MultiExtensionFuzz extends ExtensionAdaptor implements SessionChangedListener, AddonFilesChangedListener {
	 ...
	 * @param name
	 */
	public MultiExtensionFuzz(String name) {
		super(name);
	}
	...
	public void startFuzzers (ArrayList<FuzzGap> subs, FuzzProcessFactory fuzzProcessFactory) {
		fuzzerThread = new FuzzerThread(getFuzzerParam());
		fuzzerThread.setTarget(subs, fuzzProcessFactory);
	...
	protected void showFuzzDialog(Component invoker) {
		...
		if (fuzzableMessageHandler != null) {
			fuzzableMessageHandler.showFuzzDialog(invoker);
		}
	}
	...
	private PopupFuzzMenu getPopupMenuFuzz() {
		if (popupFuzzMenu== null) {
			...
			popupFuzzMenu.setText("Multi Fuzzing");
	...
	public List <String> getJBroFuzzFuzzerNames(String category) {
		String jbfCategory = category.substring(MultiExtensionFuzz.JBROFUZZ_CATEGORY_PREFIX.length());
