...
public class ExtensionWavsepReport extends ExtensionAdaptor {
	...
	private static final ImageIcon ICON = new ImageIcon(
			ExtensionWavsepReport.class.getResource( RESOURCE + "/cake.png"));
	...
	private ZapMenuItem getRunReportMenu() {
        if (runReportMenu == null) {
        	...
        	runReportMenu.addActionListener(new java.awt.event.ActionListener() {
                @Override
	...
	private int[] wavsepReport (SiteNode node) {
		if (node.getChildCount() > 0) {
			...
			getOutputPane().append(node.getHierarchicNodeName() + ", " + pass + ", " + fail + ", " + passRate + "%\n");
			...
			for (Alert alert : node.getAlerts()) {
				...
				for (String[] rule : RULES) {
					if (node.getHierarchicNodeName().contains(rule[0]) && shortForm.equals(rule[1])) {
						if (rule[2].equals(PASS)) {
							...
							System.out.println("Unextected rule " + rule[0] + " | " + rule[1] + " | " + rule[2]);
						}
						break;
					}
				}
				if (! foundRule) {
					//System.out.println("No rule for " + node.getHierarchicNodeName() + " " + shortForm);
			...
			if (pass) {
				detailHtml.append("<font style=\"BACKGROUND-COLOR: GREEN\">&nbsp;PASS&nbsp</font></td><td>");
			} else {
				detailHtml.append("<font style=\"BACKGROUND-COLOR: RED\">&nbsp;FAIL&nbsp</font></td><td>");
			}
...
//System.out.println("WavsepRpt child " + node.getHierarchicNodeName() + ", " + pass + ", " + fail);
		}
//System.out.println("WavsepRpt wavsepReport done " + node.getHierarchicNodeName() + "pass=" + pass + " fail=" + fail);
	}
	...
	private void runReport() {
		...
		SiteNode wavsepNode = this.getWavsepNode((SiteNode)Model.getSingleton().getSession().getSiteTree().getRoot());
		if (wavsepNode == null) {
		...
		try {
			...
			pw.println("<tr><th>Top Level</th><th>Pass</th><th>Fail</th><th>Score</th><th>Chart</th>");
			...
			pw.println("<table border=\"1\">");
			pw.println("<tr><th>Page</th><th>Result</th><th>Pass</th><th>Fail</th><th>Ignore</th><th>Other</th>");
