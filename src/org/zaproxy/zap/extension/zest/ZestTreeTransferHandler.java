...
public class ZestTreeTransferHandler extends TransferHandler {
    ...
    public boolean canImport(TransferHandler.TransferSupport support) {
    	// Debug logging commented out as it produces loads of messages. Useful if debugging DnD issues of course ;)
        ...
        if (parent.getUserObject() == null) {
        	...
        	//logger.debug("canImport cant paste to a node of class " + parent.getUserObject().getClass().getCanonicalName());
        	return false;
        } else {
            ...
            if (dropZew == null || ! (dropZew.getElement() instanceof ZestContainer)) {
        		// Dont support other elements yet
            	//logger.debug("canImport cant paste to a non ZestContainer " + dropZew.getElement().getClass().getCanonicalName());
            	return false;
        	} else if (dropZew.getElement() instanceof ZestConditional && dropZew.getShadowLevel() == 0) {
		...
		if (sw == null) {
			return false;
		} else if (ExtensionPassiveScan.SCRIPT_TYPE_PASSIVE.equals(sw.getTypeName()) && ! isSafe(dragStmt)) {
