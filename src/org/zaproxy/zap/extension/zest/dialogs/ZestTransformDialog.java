...
public class ZestTransformDialog extends StandardFieldsDialog {
	...
	private ScriptNode reqNode = null;
	...
	public void init (ZestScript script, ScriptNode reqNode, ZestTransformation transformation, boolean add) {
		this.add = add;
		this.script = script;
	...
	private List<String> getRequestList(ScriptNode parent, ZestStatement child, List <String> list) {
		if (list == null) {
			list = new ArrayList<String>();
		}
		// Loop up through parent nodes
		if (parent instanceof ScriptNode) {
			ScriptNode zp = (ScriptNode) parent;
			/* TODO
			if (zp.getZestElement() instanceof ZestContainer) {
				ZestContainer cont = (ZestContainer) zp.getZestElement();
				while (child != null) {
	...
	public void save() {
		...
		if (add) {
			// TODO specify parent node!
			extension.addToRequest(null, request, transformation);
		} else {
			// TODO
