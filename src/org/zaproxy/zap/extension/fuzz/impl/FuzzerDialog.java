...
public class FuzzerDialog<M extends Message, FO extends FuzzerOptions, FMP extends FuzzerMessageProcessor<M>> extends
    ...
    public FuzzerDialog(
        ...
        this.payloadGeneratorsUIHandlers = new PayloadGeneratorsContainer(PayloadGeneratorUIHandlersRegistry.getInstance()
                .getPayloadGeneratorUIHandlers(), PayloadGeneratorUIHandlersRegistry.getInstance()
        ...
        if (outgoingMessage) {
            ...
            fuzzMessagePanel.addComponent(new ResponseSplitComponent<>(), new ZapXmlConfiguration());
            HttpPanelManager.getInstance().addResponsePanel(fuzzMessagePanel);
        }
        fuzzMessagePanel.setMessage(message, true);

        fuzzLocationsPanel = new FuzzMessageLocationsPanel(this, fuzzMessagePanel, payloadGeneratorsUIHandlers);
        fuzzMessagePanel.addFocusListener(fuzzLocationsPanel.getFocusListenerAddButtonEnabler());

        optionsPanel = new FuzzerOptionsPanel<>(
    ...
    private JPanel getMessageFuzzLocationsPanel() {
        if (messageFuzzLocationsPanel == null) {
            ...
            JLabel fuzzLocationsLabel = new JLabel(
                    Constant.messages.getString("fuzz.fuzzer.dialog.messagelocations.locations.label"));
            ...
            JSplitPane splitPane = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, fuzzMessagePanel, rightPanel);
            splitPane.setResizeWeight(0.6d);

            messageFuzzLocationsPanel.add(splitPane);
            messageFuzzLocationsPanel.setBorder(BorderFactory.createEtchedBorder(EtchedBorder.RAISED));
        }
    ...
    public JButton[] getExtraButtons() {
        if (extraButtons == null) {
            JButton resetButton = new JButton(Constant.messages.getString("fuzz.fuzzer.dialog.button.reset"));
