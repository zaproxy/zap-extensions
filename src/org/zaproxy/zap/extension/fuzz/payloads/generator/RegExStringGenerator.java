...
class Parser {
    ...
    final Character[] LOWER = ArrayUtils.toObject("abcdefghijklmnopqrstuvwxyz_".toCharArray());
    final Character[] UPPER = ArrayUtils.toObject("ABCDEFGHIJKLMNOPQRSTUVWXYZ".toCharArray());
...
/**
 * An implementation of a non-deterministic finite state machine which is able to calculate all possible paths that could be
 * taken for a certain input.
 */
class NFA {
...
/**
 * Class parsing a regular expression from strings, transforming the parse-trees into NFAs and creating matching strings of a
...
public class RegExStringGenerator {
    ...
    public ArrayList<String> regexExpansion(String s, int depth, int cutoff) {
        ...
        try {
            t = p.parse(s);
            NFA d = new NFA(t);
            return d.travers(depth, cutoff - 1);
        } catch (Exception e) {
