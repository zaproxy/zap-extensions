...
public class ExtensionRequester extends ExtensionAdaptor {
	...
	public static final ImageIcon REQUESTER_ICON = new ImageIcon(
			ExtensionRequester.class.getResource( RESOURCE + "/requester.png"));
	...
	public Message getSelectedMsg() {
		...
		if (focusedComponent != null) {
			if (getView().getMainFrame().getFocusOwner().getClass().getName().startsWith("org.zaproxy.zap.extension.httppanel.view")) {		
				Component httpPanel = SwingUtilities.getAncestorOfClass(HttpPanel.class, focusedComponent);				
				if (httpPanel != null) {
	...
	private RightClickMsgMenuRequester getPopupMsgMenuRequester() {
        if (popupMsgMenuRequester == null) {
        	popupMsgMenuRequester = new RightClickMsgMenuRequester(Constant.messages.getString("requester.rightclickmenu.label"));  
