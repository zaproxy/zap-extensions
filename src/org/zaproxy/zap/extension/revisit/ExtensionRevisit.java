...
public class ExtensionRevisit extends ExtensionAdaptor implements ProxyListener {
	...
	public void hook(ExtensionHook extensionHook) {
	    ...
	    if (getView() != null) {
	    	...
	    	extensionHook.getHookMenu().addPopupMenuItem(
	    			new RightClickRevisitMenu(this, 
	    					Constant.messages.getString(PREFIX + ".popup.enable.title"), true));
	    	extensionHook.getHookMenu().addPopupMenuItem(
	    			new RightClickRevisitMenu(this, 
	...
	public boolean onHttpRequestSend(HttpMessage msg) {
		...
		if (rs != null) {
			...
			int urlCount = 0;
			try {
				node = SessionStructure.find(Model.getSingleton().getSession().getSessionId(), 
						msg.getRequestHeader().getURI(), msg.getRequestHeader().getMethod(), 
					...
					for (int i=1; i <= node.getHistoryReference().getHistoryId(); i++) {
						...
						if (hr.getHistoryType() == HistoryReference.TYPE_PROXIED &&
							...
							if (!url.equals(hr.getURI().toString())) {
								...
								if (urlCount <= 10) {
									// Add to 404 diags
									appendMsgToDiags(hr.getHttpMessage(), urlsFor404, 
							...
							if (this.isSameRequest(msg, msg2)) {
								...
								appendMsgToDiags(msg2, urlsFor404, 
										Constant.messages.getString("revisit.diags.params"));
								urlCount++;
							}
			...
			if (!found) {
				...
				try {
					msg.setResponseHeader(new HttpResponseHeader("HTTP/1.1 404 Not Found" + HttpHeader.CRLF +
							"Server: Apache-Coyote/1.1" + HttpHeader.CRLF +
							"Content-Type: text/html;charset=utf-8" + HttpHeader.CRLF +
							"Content-Language: en"));
	...
	private boolean isSameRequest(HttpMessage msg, HttpMessage msg2) {
		...
		ExtensionAntiCSRF extAcsrf = (ExtensionAntiCSRF) Control.getSingleton().getExtensionLoader().getExtension(ExtensionAntiCSRF.NAME);
		...
		ParameterParser fpp = session.getFormParamParser(msg.getRequestHeader().getURI().toString());
	...
	private void copyResponse (HttpMessage fromMsg, HttpMessage toMsg) throws HttpMalformedHeaderException {
		...
		String zapDiv = "<div id=\"zapRevisitFooter\" "
				+ "style=\"position: fixed;bottom: 0;width: 100%;text-align: center; background:rgba(255,255,255,0.8);\">";
		if (fromMsg.getResponseHeader().isHtml() &&
				body.indexOf(zapDiv) == -1) {
			int bodyIndex = body.toLowerCase().lastIndexOf("</body");
			if (bodyIndex > 0) {
				// Inject the footer
				body = body.substring(0, bodyIndex) +
						zapDiv +
						"ZAP Revisit: Page originally generated on " + fromMsg.getResponseHeader().getHeader("Date") +
						"</div>" +
