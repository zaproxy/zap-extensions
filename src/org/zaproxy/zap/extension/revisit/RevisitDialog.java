...
public class RevisitDialog extends AbstractDialog {
	...
	private final int[] DATE_FIELDS = {Calendar.YEAR, Calendar.MONTH, Calendar.DAY_OF_MONTH,
     ...
     * @throws HeadlessException
     */
    public RevisitDialog(ExtensionRevisit extension, Frame arg0, boolean arg1) throws HeadlessException {
        super(arg0, arg1);
        this.extension = extension;
        this.setAlwaysOnTop(true);
 		initialize();
	...
	public void init(SiteNode sn, Date startTime, Date endTime) {
		...
		for (int i=0; i < DATE_FIELDS.length; i++) {
			this.getStartField(i, FIELD_SIZES[i]).setText(
					String.format("%0" + FIELD_SIZES[i] + "d", startCal.get(DATE_FIELDS[i])));

			this.getEndField(i, FIELD_SIZES[i]).setText(
	...
	private JPanel getJPanel() {
		if (jPanel == null) {
	    	...
	    	jPanel.add(getHeader(), 
					LayoutHelper.getGBC(0, 0, 2, 1.0D, 0.0D));
			...
			for (int i=0; i < DATE_FIELDS.length; i++) {
				...
				ztf.setText(Constant.messages.getString("revisit.dialog.header." + HEADER_FIELDS[i]));
				ztf.setEnabled(false);
				headerPanel.add(ztf, 
						LayoutHelper.getGBC(x++, 0, 1, 0.0D, 0.0D));
				headerPanel.add(new JLabel(DATE_SEPARATORS[i]), 
						LayoutHelper.getGBC(x++, 0, 1, 0.0D, 0.0D));
			}
			...
			x=0;
			for (int i=0; i < DATE_FIELDS.length; i++) {
				startPanel.add(this.getStartField(i, FIELD_SIZES[i]), 
						LayoutHelper.getGBC(x++, 0, 1, 0.0D, 0.0D));
				startPanel.add(new JLabel(DATE_SEPARATORS[i]), 
						LayoutHelper.getGBC(x++, 0, 1, 0.0D, 0.0D));
			}
			jPanel.add(new JLabel(Constant.messages.getString("revisit.dialog.start.label")), 
			...
			x=0;
			for (int i=0; i < DATE_FIELDS.length; i++) {
				endPanel.add(this.getEndField(i, FIELD_SIZES[i]), 
						LayoutHelper.getGBC(x++, 0, 1, 0.0D, 0.0D));
				endPanel.add(new JLabel(DATE_SEPARATORS[i]), 
						LayoutHelper.getGBC(x++, 0, 1, 0.0D, 0.0D));
			}
			jPanel.add(new JLabel(Constant.messages.getString("revisit.dialog.end.label")), 
	...
	private ZapTextField getStartField (final int id, int size) {
		if (startFields[id] == null) {
			...
			startFields[id].addKeyListener(new KeyListener(){
				@Override
	...
	private ZapTextField getEndField (final int id, int size) {
		if (endFields[id] == null) {
			...
			endFields[id].addKeyListener(new KeyListener(){
				@Override
				public void keyTyped(KeyEvent e) {
				}
	...
	private Calendar getStartCal() {
		...
		for (int i=0; i < DATE_FIELDS.length; i++) {
			startCal.set(DATE_FIELDS[i], Integer.parseInt(this.getStartField(i, FIELD_SIZES[i]).getText()));
		}
		return startCal;
	}
	...
	private Calendar getEndCal() {
		...
		// Update with any user changes
		for (int i=0; i < DATE_FIELDS.length; i++) {
			endCal.set(DATE_FIELDS[i], Integer.parseInt(this.getEndField(i, FIELD_SIZES[i]).getText()));
		}
		return endCal;
	}
