...
/**
 * a scanner that looks for application source code disclosure using Git metadata/file disclosure
 * 
 * @author 70pointer
 *
 */
public class SourceCodeDisclosureGit extends AbstractAppPlugin {
	 ...
	 * the logger object
	 */
	private static Logger log = Logger.getLogger(SourceCodeDisclosureGit.class);
	...
	public void scan() {
		...
		if (log.isDebugEnabled()) {
			...
			log.debug("Checking [" + getBaseMsg().getRequestHeader().getMethod() + "] ["
					+ getBaseMsg().getRequestHeader().getURI() + "], for Source Code Disclosure using Git meta-data");
		}
	...
	private boolean findSourceCodeGit(HttpMessage originalMessage) throws Exception {
		...
		try {
			...
			URI originalURIWithoutQuery = new URI (originalURI.getScheme(), originalURI.getAuthority(), path, null, null);
			...
			while ((! modifiedpath.equals("")) && (! gitSHA1located)) {
				...
				try {
					...
					if (! gitindexentrycache.isIndexCached(gitindexuri)) {
						...
						if (log.isDebugEnabled()) log.debug("Git Index "+ gitindexuri.getURI() + " is not cached. We will parse and cache it");
						...
						if ( gitFiles != null) {
							...
							while (entryIterator.hasNext()) {
								...
								//the URIs from the Git index file do not have a query or fragment component, so no need to strip those off here
								URI gitIndexEntryUri = new URI (originalURI.getScheme(), originalURI.getAuthority(), modifiedpath + gitIndexEntry.getKey(), null, null);
								String gitSHA1Temp= gitIndexEntry.getValue();

								//cache the entry..
								if (log.isDebugEnabled()) log.debug("Caching Git Index file "+ gitindexuri.getURI() + ", Index Entry " + gitIndexEntryUri.getURI() + ", SHA1 " + gitSHA1Temp);
					...
					if ( gitindexentrycache.isIndexEntryCached(gitindexuri, originalURIWithoutQuery)) {
						...
						log.debug("Git SHA1 '"+ gitsha1+ "' was found for Git index file '"+ gitindexuri + ", Git index entry file '" + originalURIWithoutQuery+ "'");
						break;
					} else {
						log.debug("A cache entry was not found for Git index file '"+ gitindexuri + ", Git index entry file '" + originalURIWithoutQuery+ "'");
					}

				}
				catch (Exception e) {
					if (log.isDebugEnabled()) log.debug("Ignoring an error getting/parsing '"+ gitindexpath + "', while trying to find the Git SHA1 value for '"+ path + "': "+ e);
				}
				finally {
					//move to the next parent directory, by first stripping off the trailing index, and grabbing up to and including the last index
			...
			if (! Arrays.equals(disclosedData, originalMessage.getResponseBody().getBytes())) {
				...
				if (dataMatchesExtension (disclosedData, fileExtension)) {
					...
					bingo(	Alert.RISK_HIGH, 
							Alert.WARNING,
							Constant.messages.getString("ascanbeta.sourcecodedisclosure.gitbased.name"),
							Constant.messages.getString("ascanbeta.sourcecodedisclosure.desc"), 
							getBaseMsg().getRequestHeader().getURI().getURI(),
							null, //parameter being attacked: none.
							null,  //attack
							new String (disclosedData), 	//Constant.messages.getString("ascanbeta.sourcecodedisclosure.gitbased.extrainfo", filename, StringUtils.join(gitURIs,", ")),  	//extraInfo
							Constant.messages.getString("ascanbeta.sourcecodedisclosure.gitbased.soln"),
							Constant.messages.getString("ascanbeta.sourcecodedisclosure.gitbased.evidence", filename, gitURIs),
							originalMessage
							);					
					return true;
				}
