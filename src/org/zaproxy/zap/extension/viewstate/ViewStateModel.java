...
public class ViewStateModel extends AbstractHttpByteHttpPanelViewModel {
	...
	public ViewStateModel(int action, ViewStateUpdatedListener listener) {
        ...
        viewstateParams.add(new ViewState(null, "ASP", "__VIEWSTATE"));
        // For chunked ASP ViewStates
        viewstateParams.add(new ViewState(null, "ASP", "__VIEWSTATEFIELDCOUNT"));
        // For JSF ViewStates
        viewstateParams.add(new ViewState(null, "JSF", "javax.faces.ViewState")); 
	...
	public <T> T getViewStateParam(String body) {
		...
		for (ViewState vsp : viewstateParams) {
			...
			if (val != null) {
				...
				if (modelAction == ExtensionHttpPanelViewStateView.VS_ACTION_REQUEST) {
					String decVal;
					try {
					...
					if (vsp.getType().equalsIgnoreCase("ASP")) {
						viewState = (T) new ASPViewState(decVal, vsp.getName());
					}
					if (vsp.getType().equalsIgnoreCase("JSF")) {
						viewState = (T) new JSFViewState(decVal, vsp.getName());
					}
				} else {
					if (vsp.getType().equalsIgnoreCase("ASP")) {
						viewState = (T) ASPViewState.getFromSource(new Source(body));
					}
					if (vsp.getType().equalsIgnoreCase("JSF")) {
						viewState = (T) new JSFViewState(val, vsp.getName());
					}
				}
			}
		}
