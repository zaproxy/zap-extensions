...
public class ASPViewState extends ViewState {
	...
	private static Map<String, StartTag> getHiddenFields(Source source) {
		...
		for (StartTag st : result) {
			...
			String name = (st.getAttributeValue("id") == null) ? st.getAttributeValue("name")
					: st.getAttributeValue("id");

			// <input type="hidden" /> will generate a null pointer exception
			// otherwise
			if (name != null)
				stMap.put(name, st);
		}
	...
	private static String extractViewstate(Map<String, StartTag> lstHiddenFields) {
		...
		int max = Integer.parseInt(lstHiddenFields.get("__VIEWSTATEFIELDCOUNT").getAttributeValue("value"));
		numSplitFields = max;
		for (int i = 1; i < max; i++) {
	...
	public boolean hasMACtest1() {
		...
		//logger.info("hasMACtest1(): " + lastCharsBeforeMac);
	...
	public boolean hasMACtest2() {
		...
		//logger.info("hasMACtest2(): " + lastCharsBeforeMac);
	...
	public enum ViewstateVersion {
		...
		ASPNET1(1f, 1.1f, false), ASPNET2(2f, 4f, true), UNKNOWN(-1f, -1f, false);
