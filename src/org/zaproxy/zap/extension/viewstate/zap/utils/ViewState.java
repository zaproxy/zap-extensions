...
public class ViewState {
	...
	public byte[] getDecodedValue() {
		byte[] val = null;
		try {
			if (getType().equalsIgnoreCase("ASP")) {
				ASPViewState avs = (ASPViewState) this;
				val = avs.decode();
			}
			if (getType().equalsIgnoreCase("JSF")) {
			...
			System.err.println("Exception in getDecodedValue(): " + e.getMessage());
		}
		return val;
	}
	...
	public String getEncodedValue(byte[] plain) {
		...
		try {
			...
			System.err.println("Exception in getEncodedValue(): " + e.getMessage());
		}
		return val;
	}
