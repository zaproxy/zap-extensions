...
public class WSDLCustomParser {
	...
	private void parseWSDLUrl(String url) {
		...
		try {
			...
			if (httpRequest.getResponseBody() != null) {
				...
				if (content == null || content.trim().length() <= 0) {
					// if(log != null) log.warn("URL response from WSDL file request has no body
					// content.");
				} else {
					// WSDL parsing.
	...
	private void parseWSDL(Definitions wsdl, boolean sendMessages) {
		...
		for (Service service : services) {
			for (Port port : service.getPorts()) {
				...
				int soapVersion = detectSoapVersion(wsdl, soapPrefix); // SOAP 1.X, where X is represented by this
																		// variable.
	...
	private HashMap<String, String> fillParameters(Element element, String parent) {
		...
		try {
			...
			if (ct == null) {
				...
				if (ct == null)
					throw new ClassCastException("Complex Type is null after cast."); // Hashmap is empty here.
			}
			...
			if (log != null)
				log.warn("There was an error when trying to parse element " + element.getName() + " from WSDL file.",
						e);
		}
		return formParams;
	}
	...
	private HashMap<String, String> addParameter(String path, String paramType, String value) {
		...
		} else if (paramType.equals("int") || paramType.equals("double") || paramType.equals("long")) {
	...
	public HttpMessage createSoapRequest(SOAPMsgConfig soapConfig) {
		if (soapConfig == null || !soapConfig.isComplete())
			return null;
	 ...
	 */
	private void sendSoapRequest(int wsdlID, HttpMessage httpRequest, String opDisplayName, StringBuilder sb) {
		if (httpRequest == null)
			return;
