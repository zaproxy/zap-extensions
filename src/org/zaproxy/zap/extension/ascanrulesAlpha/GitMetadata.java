...
public class GitMetadata {
	 ...
	 *  http://www.example.com/.git/objects/49/a7eca74dfebcaba00ea5eee60dcff7918f930c  		- an example unpacked (aka "loose") file in the objects directory
	 *  http://www.example.com/.git/objects/info/packs										- contains the name of the pack file and index file 
	 *  http://www.example.com/.git/objects/pack/pack-ae0d45afdff3d83a8b724294aa33e617c5e3dce9.idx		- an example pack index file 
	 *  http://www.example.com/.git/objects/pack/pack-ae0d45afdff3d83a8b724294aa33e617c5e3dce9.pack		- an example pack data file
	 */
	 ...
	 * @throws Exception
	 */
	protected byte [] getURIResponseBody (URI uri, boolean inflate, HttpMessage basemsg) throws Exception {
		byte [] data = null;
		...
		uriCount++;
		if (uriCount > 20) {
			throw new Exception ("Too many Git URLs requested for this URI: " + uriCount + ". Aborting");
		}
		//TODO: split out the Git MetaData from the SourceCodeDisclosure class (not as a nested class)
		...
		if  (this.urisUsed == null || this.urisUsed.equals("")) 
			this.urisUsed = uri.getURI(); 
		else 
			this.urisUsed = this.urisUsed + ", " + uri.getURI();
	...
	public byte [] getObjectData (HttpMessage basemsg, String gitbasepath, String filesha1, boolean trypacked) throws Exception {
		...
		if (! trypacked) {
			...
			//now we're out of the pack index file version 1 or 2/3 specific stuff.. the rest of the logic is fairly common (except for the "pack" file version 4 stuff, of course! :)    			
