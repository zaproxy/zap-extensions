...
public class SQLInjectionSQLite extends AbstractAppParamPlugin {
	...
	//Some relevant notes about SQLite's support for various functions, which will affect what SQL is valid, 
	//and can be used to exploit each particular version :)
	//some of the functions noted here are *very* useful in exploiting the system hosting the SQLite instance, 
	//but I won't give the game away too easily. Go have a play!
	//3.8.6       (2014-08-15) - hex integer literals supported, likely(X) supported, readfile(X) and writefile(X,Y) supported (if extension loading enabled)
	//3.8.3       (2014-02-03) - common table subexpressions supported ("WITH" keyword), printf function supported
	...
	//3.7.17      (2013-05-20) - new loadable extensions: including amatch, closure, fuzzer, ieee754, nextchar, regexp, spellfix, and wholenumber
	...
	//3.5.7       (2008-03-17) - ALTER TABLE uses double-quotes instead of single-quotes for quoting filenames (why filenames????).
	//3.3.13      (2007-02-13) - randomBlob() and hex() supported
	//3.3.8       (2006-10-09) - IF EXISTS on CREATE/DROP TRIGGER/VIEW
	//3.3.7       (2006-08-12) - virtual tables, dynamically loaded extensions, MATCH operator supported
	...
	//3.1.0 ALPHA (2005-01-21) - ALTER TABLE ... RENAME TABLE, CURRENT_TIME, CURRENT_DATE, and CURRENT_TIMESTAMP, EXISTS clause, correlated subqueries supported
	//3.0.0 alpha (2004-06-18) - dropped support for COPY function, possibly added sqlite_source_id(), replace() (which were not in 2.8.17)
	...
	//2.5.0       (2002-06-17) - Double-quoted strings interpreted as column names not text literals, 
	//                           SQL-92 compliant handling of NULLs, full SQL-92 join syntax and LEFT OUTER JOINs supported
	...
	//2.3.3       (2002-02-18) - Allow identifiers to be quoted in square brackets, "CREATE TABLE AS SELECT" supported
	...
	//1.0.28      (2001-04-04) - special column names ROWID, OID, and _ROWID_ supported (with random values)
	//1.0.4       (2000-08-28) - length() and substr() supported
	//1.0         (2000-08-17) - covers lots of unversioned releases.  ||, fcnt(), UNION, UNION ALL, INTERSECT, and EXCEPT, LIKE, GLOB, COPY supported
	...
	private Pattern errorMessagePatterns [] = {
			Pattern.compile("no such function: randomblob", Pattern.CASE_INSENSITIVE)		//this one is specific to the time-based attack attempted here, and is indicative of SQLite versions < 3.3.13, and >= 2.4.4 (because the CASE statement is used) 
	...
	private static String SQLITE_VERSION_FUNCTIONS [] = {
														...
														,"sqlite_version()+0"  //numeric type - SQLite does implicit casting to convert "3.7.16.2" to 3.7 in numeric form.														
														,"sqlite_source_id()"	//string type - this function was added in 3.6.18.  it gets the version in the form of "2013-04-12 11:52:43 cbea02d93865ce0e06789db95fd9168ebac970c7"
																				//there may not be much point in running this one, since it gives much the same info as "sqlite_version()" and cannot be converted to an int.
																				//maybe it's useful in the case that a WAF is detecting / blocking "sqlite_version()" though? 
														};
	...
	public void scan(HttpMessage originalMessage, String paramName, String originalParamValue) {
		...
		try {
			...
			for (int timeBasedSQLindex = 0; 
					timeBasedSQLindex < SQL_SQLITE_TIME_REPLACEMENTS.length && doTimeBased && countTimeBasedRequests < doTimeMaxRequests && ! foundTimeBased ;
					timeBasedSQLindex ++
					) {
				...
				for (long numBlobsToCreate = minBlobBytes; 
					...
					//before we do the time based checking, first check for a known error message from the atatck, indicating a SQL injection vuln
					for ( Pattern errorMessagePattern : errorMessagePatterns) {
						...
						if ( errorFound ) {
							...
							bingo(Alert.RISK_HIGH, Alert.CONFIDENCE_MEDIUM, getName(), getDescription(),
									...
									errorMessagePattern.toString(), this.getCweId(), this.getWascId(), 
						            msgDelay);

							if ( this.debugEnabled ) log.debug ("A likely Error Based SQL Injection Vulnerability has been found with ["+msgDelay.getRequestHeader().getMethod()+"] URL ["+msgDelay.getRequestHeader().getURI().getURI()+"] on field: ["+paramName+"], by matching for pattern ["+ errorMessagePattern.toString()+"]");
							foundTimeBased = true;		//yeah, I know. we found an error based, while looking for a time based. bale out anyways.
					...
					//no error message detected from the time based attack.. continue looking for time based injection point.					
				...
				if (numberOfSequentialIncreases >= this.incrementalDelayIncreasesForAlert) {  
					...
					bingo(Alert.RISK_HIGH, Alert.CONFIDENCE_MEDIUM, getName(), getDescription(), 
							...
							extraInfo, getSolution(),
							extraInfo /*as evidence*/, this.getCweId(), this.getWascId(),
								...
								for ( String nextStatementCommenter: SYNTACTIC_NEXT_STATEMENT_COMMENTER) {
									for ( String statementUnionStatement: SYNTACTIC_UNION_STATEMENTS) {
										for ( String value: candidateValues) {
											...
											//TODO: fix so that the logic does not spin through the loop headers to get out of all of the nested loops..
											...
											unionAttack = unionAttack.replace("<<<<SQLITE_VERSION_FUNCTION>>>>", sqliteVersionFunction);
											unionAttack = unionAttack.replace("<<<<SYNTACTIC_PREVIOUS_STATEMENT_TYPE_CLOSER>>>>", statementTypeCloser);
											unionAttack = unionAttack.replace("<<<<SYNTACTIC_PREVIOUS_STATEMENT_CLAUSE_CLOSER>>>>", statementClauseCloser);
											unionAttack = unionAttack.replace("<<<<UNIONADDITIONALCOLUMNS>>>>", unionAdditionalColms);
											unionAttack = unionAttack.replace("<<<<SYNTACTIC_NEXT_STATEMENT_COMMENTER>>>>", nextStatementCommenter);
											unionAttack = unionAttack.replace("<<<<UNIONSTATEMENT>>>>", statementUnionStatement);
											unionAttack = unionAttack.replace("<<<<VALUE>>>>", value);
											
											if (log.isDebugEnabled()) log.debug("About to try to determine the SQLite version with [" + unionAttack + "]");										
											...
											Matcher matcher = versionNumberPattern.matcher(unionAttackMessage.getResponseBody().toString());
											while (matcher.find()) {
												...
												if ( ! matcherVersionInOriginal.find()) {
													...
													if (log.isDebugEnabled()) log.debug ("We found SQLite version ["+ versionNumber + "]");
