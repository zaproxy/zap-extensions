 ...
 *
 * <p>AUTHOR: GORAN SARENKAPA - JordanGS SPONSOR: RYERSON UNIVERSITY
 */
    ...
    public ApiResponse handleApiAction(String name, JSONObject params) throws ApiException {
        ...
        switch (name) {
            case ACTION_GENERATE:
                ...
                if (!extension.canWrite(absolutePath)) {
                    logger.warn(
                            Constant.messages.getString(
                                    "exportreport.message.console.error.file.writable",
                                    absolutePath));
                ...
                if (logger.isDebugEnabled()) {
                    logger.debug(
                            Constant.messages.getString("exportreport.message.console.info.path"));
                }
                ...
                ArrayList<String> alertDetailsFlags =
                        new ArrayList<String>(
                                Arrays.asList(
                                        (params.getString(ACTION_PARAM_ALERT_DETAILS)).split(";")));
                ...
                if (sourceDetails.size() != ExtensionExportReport.SOURCE_COUNT) {
                    logger.error(
                            Constant.messages.getString(
                                    ...
                                    sourceDetails.size(),
                                    ExtensionExportReport.SOURCE_COUNT,
                                    ...
                                    Constant.messages.getString(
                                            "exportreport.source.scandate.label"),
                                    Constant.messages.getString(
                                            "exportreport.source.reportdate.label"),
                                    Constant.messages.getString(
                                            "exportreport.source.scanver.label"),
                                    Constant.messages.getString(
                                            "exportreport.source.reportver.label"),
                                    Constant.messages.getString(
                ...
                if (alertSeverityFlags.size() != extension.getAlertSeverity().size()) {
                    logger.error(
                            Constant.messages.getString(
                                    ...
                                    Constant.messages.getString(
                                            "exportreport.risk.severity.low.label"),
                                    Constant.messages.getString(
                ...
                if (alertDetailsFlags.size() != extension.extensionGetMaxList()) {
                    logger.error(
                            Constant.messages.getString(
                                    ...
                                    extension.extensionGetMaxList(),
                                    Constant.messages.getString("exportreport.details.cweid.label"),
                                    Constant.messages.getString(
                                            "exportreport.details.wascid.label"),
                                    Constant.messages.getString(
                                            "exportreport.details.description.label"),
                                    Constant.messages.getString(
                                            "exportreport.details.otherinfo.label"),
                                    Constant.messages.getString(
                                            "exportreport.details.solution.label"),
                                    Constant.messages.getString(
                                            "exportreport.details.reference.label"),
                                    Constant.messages.getString(
                ...
                if (!extension.validList(alertDetailsFlags)) {
                    logger.warn(
                            Constant.messages.getString(
                                    "exportreport.message.console.error.valid.list",
                                    Constant.messages.getString(
                                            "exportreport.menu.details.label")));
                    return ApiResponseElement.FAIL;
                }
                if (logger.isDebugEnabled()) {
                    logger.debug(
                ...
                alertDetailsFull.addAll(
                        extension.getAlertDetails().size(), extension.getAlertAdditional());
                ArrayList<String> alertDetailsTemp =
                        extension.generateList(alertDetailsFlags, alertDetailsFull);

                try {
                    if (extension.generateReport(
                            absolutePath,
                            fileExtension,
                            sourceDetails,
                            alertSeverityTemp,
                        ...
                        if (logger.isDebugEnabled()) {
                            logger.debug(
                                    Constant.messages.getString(
                                            "exportreport.message.console.info.pass.path",
                                            absolutePath));
                        }
