...
public class PanelSource extends JPanel {
    ...
    private void initialize() {
        ...
        String strDescription =
                Constant.messages.getString("exportreport.source.description.label");
        String ttDescription =
                Constant.messages.getString("exportreport.source.description.tooltip");
        ...
        txtTitle =
                SharedFunctions.createTextField(
                        content,
                        Model.getSingleton().getSession().getSessionName().toString(),
                        ttTitle,
                        true,
        ...
        txtBy =
                SharedFunctions.createTextField(
                        content, "", ttBy, true, extension.getTextfieldLimit());

        SharedFunctions.createLabel(content, strFor, SharedFunctions.getLabelFont());
        txtFor =
                SharedFunctions.createTextField(
                        content, "", ttFor, true, extension.getTextfieldLimit());

        String date =
                SharedFunctions
                        .getCurrentTimeStamp(); // Set the Scan date as the current timestamp to
        // give user idea of format.
        ...
        txtReportDate =
                SharedFunctions.createTextField(
                        content, date, ttReportDate, false, extension.getTextfieldNoLimit());

        SharedFunctions.createLabel(content, strScanVer, SharedFunctions.getLabelFont());
        txtScanVer =
                SharedFunctions.createTextField(
                        content,
                        Constant.messages.getString("exportreport.message.notice.notavailable"),
                        ttScanVer,
                        false,
        ...
        txtDescription =
                SharedFunctions.createTextArea(2, 0, ttDescription, extension.getTextareaLimit());
        JScrollPane sp = new JScrollPane(txtDescription);
        content.add(sp);

        SpringUtilities.makeCompactGrid(
                content, 8, 2, 6, 6, 6,
                6); // Lay out the panel, rows, cols, initX, initY, xPad, yPad
    }
