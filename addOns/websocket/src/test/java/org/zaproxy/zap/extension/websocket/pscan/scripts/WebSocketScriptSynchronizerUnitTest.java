...
class WebSocketScriptSynchronizerUnitTest extends TestUtils {
    ...
    private GspmRegistry gspmRegistry;
    ...
    void setUp() throws Exception {
        ...
        ExtensionCommonlib commonlib = mock(ExtensionCommonlib.class);
        gspmRegistry = mock(GspmRegistry.class);
        lenient().when(commonlib.getGspmRegistry()).thenReturn(gspmRegistry);
        lenient().when(loader.getExtension(ExtensionCommonlib.class)).thenReturn(commonlib);
    ...
    @Test
    void shouldNotLoopWhenDisablingClashingScriptReentersSynchronizer() throws Exception {
        // Given
        // Mimics ExtensionScript#setEnabled firing a script changed event synchronously, which
        // WebSocketPassiveScriptEventListener forwards straight back into scriptAdded(script).
        var metadata = new ScanRuleMetadata(12345, "Copy of Test Scan Rule");
        ScriptWrapper script = createScriptWrapper(metadata);
        WebSocketPassiveScanner existing = mock(WebSocketPassiveScanner.class);
        given(existing.getId()).willReturn(12345);
        given(existing.getName()).willReturn("Existing Rule");
        ...
        // Then
        verify(scannerManager, never()).add(any());
        verify(extensionScript, times(1)).setEnabled(script, false);
    }

    @Test
    void shouldNotLoadScriptWithIdClashingAcrossOtherGspmTools() throws Exception {
        // Given
        var metadata = new ScanRuleMetadata(12345, "Test Scan Rule");
        ScriptWrapper script = createScriptWrapper(metadata);
        given(scannerManager.getScanners()).willReturn(List.of());
        GspmRule existingRule = mock(GspmRule.class);
        given(existingRule.getName()).willReturn("Active Scan Rule");
        given(gspmRegistry.getRule(12345)).willReturn(Optional.of(existingRule));
