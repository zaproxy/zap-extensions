...
public class WebSocketPanel extends AbstractPanel implements WebSocketObserver {
    ...
    public static final int WEBSOCKET_OBSERVING_ORDER =
            WebSocketStorage.WEBSOCKET_OBSERVING_ORDER + 5;
    ...
    public WebSocketPanel(
            TableWebSocket webSocketTable, WebSocketBreakpointsUiManagerInterface brkManager) {
        super();
    ...
    /**
     * Lazy initializes header of this WebSocket tab with a select box and a filter.
     *
     * @return
     */
    private Component getPanelToolbar() {
        if (panelToolbar == null) {
            ...
            constraints.gridx = x++;
            panelToolbar.add(
                    new JLabel(Constant.messages.getString("websocket.toolbar.channel.label")),
                    constraints);
    ...
    /**
     * Lazy initializes the part of the WebSockets tab that is used to display the messages.
     *
     * @return
     */
    ...
    public WebSocketMessagesViewFilterDialog getFilterDialog() {
        if (filterDialog == null) {
            filterDialog =
                    new WebSocketMessagesViewFilterDialog(View.getSingleton().getMainFrame(), true);
        }
        return filterDialog;
    }
