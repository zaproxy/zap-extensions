 ...
 * href="http://tools.ietf.org/html/rfc6455">RFC6455</a>. Code was inspired by the <a
...
public class WebSocketProxyV13 extends WebSocketProxy {
     ...
     * represented in Java by <code>new BigInteger("9223372036854775806")</code>. But for this
    ...
    @Override
    protected WebSocketMessage createWebSocketMessage(WebSocketMessageDTO message)
            throws WebSocketException {
    ...
    protected class WebSocketMessageV13 extends WebSocketMessage {
        ...
        private class WebSocketFrameV13 {
            ...
            /**
             * After a frame is sealed, it cannot be changed and data can be read by {@link
             * WebSocketFrameV13#getBuffer()}.
             */
            ...
            /**
             * Builds up a frame according to given payload. Sets header and metadata (opcode,
             * payload length, mask).
             *
             * @param payload
             */
            ...
            public void reallocateFor(int bytesRead) throws WebSocketException {
                if (isSealed) {
                    throw new WebSocketException(
                            "You cannot change size of 'sealed' frame's buffer.");
                }
            ...
            public byte[] getBuffer() throws WebSocketException {
                if (!isSealed) {
                    throw new WebSocketException(
                            "You should call seal() on WebSocketFrame first, before getBuffer().");
                }
        ...
        /**
         * Temporary buffer that will be added to {@link WebSocketMessage#receivedFrames} as soon as
         * whole frame is read.
         */
        ...
        /**
         * By default, there are 7 bits to indicate the payload length. If the length can not be
         * shown with 7 bits, the payload length is set to 126. Then the next 16 bits interpreted as
         * unsigned integer is the payload length.
         */
        private static final int PAYLOAD_LENGTH_16 = 126;

        /**
         * If the 7 bits represent the value 127, then the next 64 bits interpreted as an unsigned
         * integer is the payload length (the most significant bit MUST be 0).
         */
         ...
         * @throws IOException
         */
        public WebSocketMessageV13(WebSocketProxy proxy, InputStream in, byte frameHeader)
                throws IOException {
        ...
        /**
         * Add next frame to this message. First byte already given by parameter
         * <em>frameHeader</em>.
         *
         * @param in
         * @param frameHeader
        ...
        /**
         * Given an {@link InputStream} and the first byte of a frame, this method reads the second
         * byte until the end of the frame.
         *
         * @param in
         * @param frameHeader
         * @throws IOException
        ...
        /**
         * Looks at the payload byte from the WebSockets header and determines the packets length.
         * It reads bytes from the extended length field if required.
         *
         * @param in
         * @param payloadByte
         * @return
        ...
        /**
         * Takes the payload of a close frame and transforms the 2 bytes representation of the
         * status code to a 4 digit string representation readable by humans.
         *
         * @param payload
         * @param statusCode
         * @return
        ...
        /**
         * Takes the modified payload of a close frame and transforms the readable status code (4
         * digit string) back to 2 byte representation.
         *
         * @param payload
         * @return
         * @throws WebSocketException
         ...
         */
        private ByteBuffer getTransmittableCloseFramePayload(ByteBuffer payload)
        ...
        public boolean forward(OutputStream out) throws IOException {
            ...
            if (hasChanged) {
                ...
                do {
                    ...
                    frameLength =
                            Math.min(PAYLOAD_MAX_FRAME_LENGTH, payload.limit() - writtenBytes);
        ...
        public void setReadablePayload(String newReadablePayload) throws WebSocketException {
            ...
            if (isValidUtf8Payload
                    && !Arrays.equals(
                            newBytesPayload, Utf8Util.decodePayloadFromUtf8(readablePayload))) {
