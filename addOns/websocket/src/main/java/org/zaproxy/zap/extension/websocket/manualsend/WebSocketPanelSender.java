...
public class WebSocketPanelSender implements MessageSender, WebSocketObserver {
	 ...
	 * @throws RequestOutOfScopeException if url it's out of scope ore or ZAP runs to safe or protected mode
     ...
     */
    public int reOpenChannel(Integer channelId, String newSecWebSocketKey, boolean followRedirects, boolean useSessionState) throws RequestOutOfScopeException, IOException {
		...
		if(newSecWebSocketKey != null){
			HttpHandshakeBuilder.setWebSocketKey(handshakeConfig.getHttpMessage().getRequestHeader(), newSecWebSocketKey);
		}else{
			HttpHandshakeBuilder.setGeneratedWebSocketKey(handshakeConfig.getHttpMessage().getRequestHeader());
		}
		
		WebSocketProxy newWebSocketConnection = connectionToEstablish.reEstablishConnection(handshakeConfig);
	 ...
	 */
	public String getWebSocketKey(Integer channelId) throws DatabaseException, HttpMalformedHeaderException {
		return getWebSocketProxy(channelId).getHandshakeReference().getHttpMessage().getRequestHeader().getHeader(HttpHandshakeBuilder.SEC_WEB_SOCKET_KEY);
	}
	...
	private ExtensionHistory getHistoryExtension() {
		if (historyExtension == null) {
			historyExtension = Control.getSingleton().getExtensionLoader().getExtension(ExtensionHistory.class);
		}
