...
public class RootContent extends WebSocketContent {
    ...
    private RootContent(RootContent that) {
        this.name = that.getName();
    }

    @Override
    public WebSocketContent clone() {
        return new RootContent(this);
    }
    ...
    public List<TreeNode<WebSocketContent>> getHostNodes(
        ...
        thisNode.applyToChildren(t -> t.getHostNodes(t, hostNodesList));
    ...
    /**
     * Iterates over it's children to retrieve channels per host node folder.
     *
     * @param thisNode The node which calls the method.
    ...
    /**
     * Iterate over it's children to retrieve {@link HistoryReference}, from the handshake messages
     * per host node folder
     *
     * @param thisNode The node which calls the method.
     * @param refMap map where it should store the values.
     * @return The map with {@link HistoryReference} per host folder.
     */
    @Override
