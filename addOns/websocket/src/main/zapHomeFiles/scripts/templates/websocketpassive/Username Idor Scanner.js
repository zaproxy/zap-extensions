  ...
  usersList.forEach(function (user) {
    ...
    Object.keys(usernameHashes).forEach(function (hashType) {
      if ((matches = message.match(usernameHashes[hashType])) != null) {
        var contextname = model
          .getSession()
          .getContext(parseInt(user.getContextId()))
          .getName();
...
function createAlertBuilder(helper, evidence, username, contextname, hashType) {
  return helper
    ...
    .setSolution(
      "Use per user or session indirect object references (create a temporary mapping at time of use)." +
...
function getExampleAlerts() {
  return [
    createAlertBuilder(
      WebSocketPassiveScript.getExampleHelper(),
      "",
      "Example",
      "Example",
      "Example",
...
function getDescription(username, contextname, hashType) {
  return (
    ...
    " / context: " +
    contextname +
    "} was found in incoming WebSocket message." +
...
function getUsers() {
  if ((extUserManagement = getExtensionUserManagement()) != null) {
    ...
    for (var i = 0; i < contexts.size(); i++) {
      ...
      var contextUsers = extUserManagement
