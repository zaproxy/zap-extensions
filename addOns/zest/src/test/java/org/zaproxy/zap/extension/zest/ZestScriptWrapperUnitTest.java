...
class ZestScriptWrapperUnitTest {
    ...
    private ScriptWrapper createMockScriptWrapper() {
        ...
        org.zaproxy.zap.extension.script.ScriptType scriptType = mock();
        given(scriptType.getName())
                .willReturn(org.zaproxy.zap.extension.script.ExtensionScript.TYPE_STANDALONE);
        given(originalScript.getContents()).willReturn("{}");
        given(originalScript.getName()).willReturn("test");
        given(originalScript.getDescription()).willReturn("test description");
        given(originalScript.getType()).willReturn(scriptType);
        given(originalScript.getEngine()).willReturn(mock());
        given(originalScript.getEngineName()).willReturn("Mozilla Zest");
        given(originalScript.isEnabled()).willReturn(true);
        given(originalScript.getFile()).willReturn(null);
        given(originalScript.isLoadOnStart()).willReturn(false);
        given(originalScript.isChanged()).willReturn(false);
        given(originalScript.getWriter()).willReturn(null);
