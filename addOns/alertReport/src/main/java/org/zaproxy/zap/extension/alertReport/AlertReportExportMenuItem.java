...
public class AlertReportExportMenuItem extends ExtensionPopupMenuItem {
        ...
        if (!filename.isEmpty()) {
            ...
            if (!alerts.isEmpty()) {
                if (extension.getParams().getFormatReport().equals("PDF"))
                    result = reportExportPDF.exportAlert(alerts, filename, extension);
                else result = reportExportODT.exportAlert(alerts, filename, extension);
                if (result)
                    View.getSingleton()
                            .showMessageDialog(
                                    extension
                                            .getMessages()
                                            .getString("alertreport.export.message.export.ok"));
                else
                    View.getSingleton()
                            .showMessageDialog(
                                    extension
                                            .getMessages()
                                            .getString("alertreport.export.message.export.fail"));
            }
    ...
    private boolean checkDuplicateAlert(java.util.List<List<Alert>> alerts, Alert alert) {
        ...
        for (int i = 0; i < alerts.size(); i++) {
            ...
            if (listAlert.contains(alert)) return true;
        }
        return result;
    }
