...
/** Unit test for {@link BruteForceParam}. */
class BruteForceParamUnitTest {

    private BruteForceParam param;
    ...
    void shouldHaveConfigVersionKey() {
        assertThat(param.getConfigVersionKey(), is(equalTo("bruteforce[@version]")));
    }
    ...
    void shouldLoadThreadsFromConfig(int threads) {
        // Given
        configuration.setProperty("bruteforce.threadPerHost", threads);
        // When
        param.load(configuration);
        // Then
    ...
    void shouldUpdateDefaultThreads() {
        try (MockedStatic<Constants> constants = mockStatic(Constants.class)) {
            ...
            configuration = new ZapXmlConfiguration();
            configuration.setProperty("bruteforce.threadPerHost", 10);
            // When
            param.load(configuration);
            // Then
    ...
    void shouldNotUpdateNonDefaultThreads() {
        try (MockedStatic<Constants> constants = mockStatic(Constants.class)) {
            ...
            constants.when(Constants::getDefaultThreadCount).thenReturn(8);
            configuration = new ZapXmlConfiguration();
            configuration.setProperty("bruteforce.threadPerHost", 3);
            // When
            param.load(configuration);
            // Then
