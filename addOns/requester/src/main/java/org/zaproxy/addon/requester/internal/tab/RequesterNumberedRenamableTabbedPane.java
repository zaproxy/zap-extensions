...
import org.zaproxy.addon.requester.internal.db.RequesterTabRecord;
...
import org.zaproxy.addon.requester.internal.util.RequesterMessageConverter;
import org.zaproxy.addon.requester.internal.util.RequesterUtil;
...
public class RequesterNumberedRenamableTabbedPane extends NumberedRenamableTabbedPane {
    ...
    private void createEditorPanel(Message message, RequesterTabRecord tabRecord) {
        RequestTabHttpEditorWrapper editorPanel =
                new RequestTabHttpEditorWrapper(tabStorage, tabRecord);
    ...
    private void initializeTabs() {
        ...
        for (RequesterTabRecord tabRecord : tabRecords) {
            Message message =
                    RequesterMessageConverter.toMessage(
                            tabRecord.getMessage(), tabRecord.getMessageType());
            createEditorPanel(message, tabRecord);
        }
    }
}
