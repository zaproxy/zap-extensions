    ...
    public AuthenticationData(Context context, List<UserData> users) {
        ...
        if (authMethod instanceof HttpAuthenticationMethod) {
            ...
            setMethod(METHOD_AUTO);
                ...
                && authMethod.getClass().getCanonicalName().equals(OAUTH2_AUTH_METHOD_CLASSNAME)) {
            // Plan to change once the core supports dynamic methods better
            setMethod(METHOD_OAUTH2);
            JobUtils.addPrivateField(parameters, PARAM_GRANT_TYPE, authMethod);
            JobUtils.addPrivateField(parameters, PARAM_TOKEN_ENDPOINT, authMethod);
            JobUtils.addPrivateField(parameters, PARAM_CLIENT_ID, authMethod);
            JobUtils.addPrivateField(parameters, PARAM_CLIENT_SECRET, authMethod);
            JobUtils.addPrivateField(parameters, PARAM_CLIENT_AUTH_METHOD, authMethod);
            JobUtils.addPrivateField(parameters, PARAM_SCOPE, authMethod);
            JobUtils.addPrivateField(parameters, PARAM_ACCESS_TOKEN_FIELD, authMethod);
            JobUtils.addPrivateField(parameters, PARAM_REFRESH_TOKEN_FIELD, authMethod);
            JobUtils.addPrivateField(parameters, PARAM_EXTRA_TOKEN_PARAMS, authMethod);
            JobUtils.addPrivateField(parameters, PARAM_VERIFICATION_URL, authMethod);
            JobUtils.addPrivateField(parameters, PARAM_DIAGNOSTICS, authMethod);
        }
        if (authMethod != null) {
            ...
            switch (getMethod().toLowerCase(Locale.ROOT)) {
                ...
                case AuthenticationData.METHOD_OAUTH2:
                    ...
                    if (oauth2Type != null) {
                        ...
                        setPrivateStringIfPresent(
                                am, AuthenticationData.PARAM_VERIFICATION_URL, env);
