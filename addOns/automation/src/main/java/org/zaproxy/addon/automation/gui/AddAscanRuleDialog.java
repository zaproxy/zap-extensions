...
public class AddAscanRuleDialog extends StandardFieldsDialog {
    ...
    private static final String I18N_PREFIX = "ascan.options.level.";

    private ActiveScanJob.Rule rule;
    private int tableIndex;
    private AscanRulesTableModel model;
    private Map<String, String> i18nToStr = new HashMap<>();;
    ...
    public AddAscanRuleDialog(AscanRulesTableModel model, ActiveScanJob.Rule rule, int tableIndex)
        ...
        if (rule != null) {
            thresholdName = this.strToI18n(rule.getThreshold());
        }

        String strengthName = "";
        if (rule != null) {
            strengthName = this.strToI18n(rule.getStrength());
        }
        ...
        for (AlertThreshold at : AlertThreshold.values()) {
            ...
            allthresholds.add(strToI18n(at.name()));
        }
        ...
        for (AttackStrength at : AttackStrength.values()) {
            i18nToStr.put(this.strToI18n(at.name()), at.name());
            allstrengths.add(strToI18n(at.name()));
        }
    ...
    public void save() {
        if (rule == null) {
            ...
            // TODO may be one or other, so ignore defaults
            rule.setThreshold(i18nToStr.get(this.getStringValue(THREHOLD_PARAM)));
            rule.setStrength(i18nToStr.get(this.getStringValue(STRENGTH_PARAM)));
            this.model.add(rule);
        } else {
            // TODO may be one or other, so ignore defaults
            rule.setThreshold(i18nToStr.get(this.getStringValue(THREHOLD_PARAM)));
    ...
    private String strToI18n(String str) {
        return Constant.messages.getString(I18N_PREFIX + str.toLowerCase(Locale.ROOT));
    }
}
