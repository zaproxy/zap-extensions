...
class ProxyListenerScriptUnitTest extends TestUtils {

    private static final String SCRIPT_TYPE = ExtensionScriptsUI.TYPE_PROXY;
    ...
    void shouldCreateScriptsCacheWithExpectedConfiguration() {
        ...
        // Then
        ArgumentCaptor<Configuration<ProxyScript>> argumentCaptor = ArgumentCaptor.captor();
    ...
    private static <T> CachedScript<T> createCachedScript(T script, ScriptWrapper scriptWrapper) {
        CachedScript<T> cachedScript = mock(withSettings().strictness(Strictness.LENIENT));
