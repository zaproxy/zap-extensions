...
class HttpSenderScriptListenerUnitTest extends TestUtils {

    private static final String SCRIPT_TYPE = ExtensionScriptsUI.TYPE_HTTP_SENDER;
    ...
    void shouldCreateScriptsCacheWithExpectedConfiguration() {
        ...
        new HttpSenderScriptListener(extensionScript);
        // Then
        ArgumentCaptor<Configuration<HttpSenderScript>> argumentCaptor = ArgumentCaptor.captor();
    ...
    void shouldRefreshScriptsAndCallOnHttpRequestSend() throws Exception {
        ...
        ArgumentCaptor<HttpSenderScriptHelper> argumentCaptor = ArgumentCaptor.captor();
    ...
    private <T> ScriptsCache<T> createScriptsCache(T script) {
        ScriptsCache<T> scriptsCache = mock();
        Answer<Void> answer =
                invocation -> {
