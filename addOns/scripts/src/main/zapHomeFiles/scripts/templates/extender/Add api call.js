...
var apiimpl = new (Java.extend(apiimpltype))({
  ...
  handleApiView: function (name, params) {
    if (name == "exampleView") {
      print(
        "handleApiView called for " +
          name +
          " mandParam = " +
          params.getString("mandParam"),
      );
      var apiresptype = Java.type(
        "org.zaproxy.zap.extension.api.ApiResponseElement",
      );
...
apiimpl.addApiView(
  new org.zaproxy.zap.extension.api.ApiView(
    "exampleView",
    ["mandParam"],
    ["optParam1", "optParam2"],
  ),
);
apiimpl.addApiAction(
  new org.zaproxy.zap.extension.api.ApiAction(
    "exampleAction",
    ["mandParam"],
    ["optParam1", "optParam2"],
  ),
