...
public class InvokeParam extends AbstractParam {
    ...
    public void setListInvoke(List<InvokableApp> listInvoke) {
        ...
        for (int i = 0, size = listInvoke.size(); i < size; ++i) {
            ...
            getConfig()
                    .setProperty(
                            elementBaseKey + APP_DIRECTORY_KEY,
            ...
            getConfig()
                    .setProperty(
                            elementBaseKey + APP_OUTPUT_KEY,
                            Boolean.valueOf(app.isCaptureOutput()));
            getConfig()
                    .setProperty(
                            elementBaseKey + APP_NOTE_KEY, Boolean.valueOf(app.isOutputNote()));

            getConfig()
                    .setProperty(
                            elementBaseKey + APP_ENABLED_KEY, Boolean.valueOf(app.isEnabled()));
