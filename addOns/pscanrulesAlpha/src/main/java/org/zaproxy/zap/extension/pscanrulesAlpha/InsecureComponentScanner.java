...
public class InsecureComponentScanner extends PluginPassiveScanner {
    ...
    private Pattern SERVER_HEADER_PATTERN_ORACLE =
            Pattern.compile("^Oracle-Application-Server-[0-9]+[gi] ([^ ]+)/([^ ]+).*$");
    ...
    private Pattern SERVER_HEADER_PATTERN_JBOSS =
            Pattern.compile("^Servlet [^ ]+[ ]+(JBoss)-([^ /]+).*$");

    /** a pattern for identifying and parsing the Generator tag in the body (normal case) */
    private Pattern BODY_GENERATOR_PATTERN =
            Pattern.compile(
                    "<\\s*meta\\s+name\\s*=\\s*\"generator\"\\s+content\\s*=\\s*\"([^\\\"]+)\\s+([0-9.]+)\"\\s*/>");

    /**
     * used to match Apache Tomcat version information, which is not leaked in the headers, but in
     * error pages only
     */
    private Pattern SERVER_BODY_PATTERN_TOMCAT =
            Pattern.compile("<html><head><title>Apache (Tomcat)/([.0-9]+) -.*</title>");
    ...
    public void scanHttpResponseReceive(HttpMessage msg, int id, Source source) {
        ...
        for (String header : serverlikeHeaderVector) {
            ...
            if (header != null) {
                // per rfc2616, the server token can contain multiple product tokens, delimited by a
                // space character
                ...
                // we're only interested cases where we can see the version, or the subcomponent
                // (since we can't do anything meaningful without a software version)
                // In the case of Apache, the subcomponent (if present) is the web server's
                // underlying OS.  That's good to know :)
                ...
                if (serverheadermatcher.matches()) {
                    ...
                    // tweak for PHP (for which the general format is RFC compliant), since the the
                    // product information stored in the
                    // database is limited to 3 levels of decimal digits, separated by dots,
                    // possibly followed directly by
                    // Alpha, Beta, RC, Patch, PL (ie, alphabetic), possibly followed by some
                    // decimal digits
                    ...
                    // Red Hat and Red Hat derived distros such as CentOS use "security backports",
                    // so that an earlier Apache version number
                    // is retained, but the product is *not* vulnerable to known security issues for
                    // that (earlier) product version.
                    String distro = "";
                    if (header.startsWith("Apache") && dregs != null && dregs.length() > 0) {
                        Matcher apacheDistroMatcher =
                                SERVER_HEADER_APACHE_DISTRO_PATTERN.matcher(dregs);
                        if (apacheDistroMatcher.matches()) {
                            distro = apacheDistroMatcher.group(1);
                            if (log.isDebugEnabled())
                                log.debug("Apache distro [" + distro + " found");
                        }
                    }
                    // record it..
                    matchingProducts.add(
                            new Product(
                                    Product.ProductType.PRODUCTTYPE_WEBSERVER,
                                    distro,
                                    product,
                                    version));
                    ...
                    if (dregs != null && dregs.length() > 0 && header.startsWith("Apache")) {
                        for (String potentialModule : dregs.split(" ")) {
                            ...
                            if (modulematcher.matches()) {
                                ...
                                // remove the leading "v" in the version for the Apache Perl module
                                // (not for mod_perl though)
                                if (product.equals("Perl"))
                                    version = version.replaceFirst("^v(.*)$", "$1");
                                matchingProducts.add(
                                        new Product(
                                                Product.ProductType.PRODUCTTYPE_APACHE_MODULE,
                                                distro,
                                                product,
                                                version));
                    ...
                    if (log.isDebugEnabled())
                        log.debug(
                                "Header "
                                        + header
                                        + " does not match the server header standard, and will be ignored");
                }

                // handle the "Generator" matches here
                // According to the following page, the generator is "one of the software packages
                ...
                while (matcherGenerator.find()) {
                    ...
                    String evidence =
                            matcherGenerator.group(
                                    0); // the evidence is not in the header, so grab it from here
                    // instead
                    matchingProducts.add(
                            new Product(
        ...
        for (String header : vialikeHeaderVector) {
            ...
            if (header != null) {
                ...
                if (viaheadermatcher.matches()) {
                    ...
                    matchingProducts.add(
                            new Product(
                                    Product.ProductType.PRODUCTTYPE_PROXY_SERVER,
                                    "",
                                    product,
                                    version));
                    matchingProductsEvidence.add(evidence);
        ...
        for (Product matchingProduct : matchingProducts) {
            ...
            if (product != null && version != null) {
                // TODO: handle special cases of web server software here that does not follow
                // rfc2616
                if (log.isDebugEnabled())
                    log.debug(
                            "Found '"
                ...
                try {
                    ...
                    // if we found vulnerabilities, raise them (by throwing a single alert, using
                    // the highest risk noted)
                    if (vulnlist != null && vulnlist.size() > 0) {
                        if (log.isDebugEnabled())
                            log.debug("Found " + vulnlist.size() + " vulnerabilities");
                        ...
                        // now we have the list of vulnerabilities in string form, so raise the
                        // alert
                        ...
                        // the confidence depends on the distro.  Red Hat and CentOS Apache versions
                        // are unreliable, for instance
                        // in that case, raise them, for visibility, but flag it with low
                        // confidence.
                        // The user will just have to check it out manually, since there is no way
                        // for us to be at all sure here.
                        ...
                        alert.setDetail(
                                ...
                                Constant.messages.getString(
                                        MESSAGE_PREFIX + "soln", product, version),
                                Constant.messages.getString(MESSAGE_PREFIX + "refs", refs),
                                evidence, // evidence
                                829, // CWE 829: Inclusion of Functionality from Untrusted Control
                                // Sphere
