 ...
 */
public class RetrievedFromCacheScanner extends PluginPassiveScanner{

	private static final String MESSAGE_PREFIX = "pscanalpha.retrievedfromcache.";
	...
	public void scanHttpResponseReceive(HttpMessage msg, int id, Source source) {
		...
		try {
			if (logger.isDebugEnabled()) logger.debug("Checking URL "+ msg.getRequestHeader().getURI().getURI() + " to see if was served from a shared cache");

			//X-Cache: HIT
			//X-Cache: HIT from cache.kolich.local					<-- was the data actually served from the cache (subject to no-cache, expiry, etc)? 
			//															(if X-Cache: HIT, it implies X-Cache-Lookup: HIT)
			//															(and if X-Cache-Lookup: MISS, it implies X-Cache: MISS)
			//X-Cache-Lookup: HIT from cache.kolich.local:80		<-- was the data *available* in the cache? (not whether it was actually served)
			...
			if (xcacheHeaders!=null) {
				for (String xcacheHeader : xcacheHeaders) {					
					for (String proxyServerDetails: xcacheHeader.split(",")) {
						...
						if ( proxyServerDetails.startsWith(" ")) proxyServerDetails=proxyServerDetails.substring(1);
						if (logger.isTraceEnabled()) logger.trace("Proxy HIT/MISS details ["+ proxyServerDetails + "]");
			...
			//The "Age" header (defined in RFC 7234) conveys the sender's estimate of the amount of time since the response (or its revalidation) was generated at the origin server.
			...
			//lets validate that it is actually a non-negative decimal integer, as mandated by RFC 7234, however.
			//if there are multiple "Age" headers, just look for one valid value in the multiple "Age" headers.. Not sure if this case is strictly valid with the spec, however.
			//Note: HTTP/1.0 caches do not implement "Age", so the absence of an "Age" header does *not* imply that the response was served from the origin server, rather than a cache..
			Vector<String> ageHeaders = msg.getResponseHeader().getHeaders("Age");
			if (ageHeaders!=null) {
				for (String ageHeader : ageHeaders) {
					if (logger.isTraceEnabled()) logger.trace("Validating Age header value ["+ ageHeader+ "]");
					...
					if (ageAsLong!= null && ageAsLong >= 0) {
						...
						alert.setDetail(
								...
								"",	// Param
								"", // Attack
								Constant.messages.getString(MESSAGE_PREFIX + "extrainfo.http11ageheader"), // Other info: "Age" header implies a HTTP/1.1 compliant cache server.
								getSolution(), //Solution
								getReference(), //References
								evidence,	// Evidence
								0,	// 0 - not necessarily an issue at all
