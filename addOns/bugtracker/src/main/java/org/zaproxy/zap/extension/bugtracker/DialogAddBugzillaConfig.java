...
class DialogAddBugzillaConfig extends AbstractFormDialog {
    ...
    private static final String DIALOG_TITLE =
            Constant.messages.getString("bugtracker.trackers.bugzilla.dialog.config.add.title");

    private static final String CONFIRM_BUTTON_LABEL =
            Constant.messages.getString(
                    "bugtracker.trackers.bugzilla.dialog.config.add.button.confirm");

    private static final String NAME_FIELD_LABEL =
            Constant.messages.getString(
                    "bugtracker.trackers.bugzilla.dialog.config.field.label.name");
    private static final String PASSWORD_FIELD_LABEL =
            Constant.messages.getString(
                    "bugtracker.trackers.bugzilla.dialog.config.field.label.password");
    private static final String BUGZILLA_URL_FIELD_LABEL =
            Constant.messages.getString(
                    "bugtracker.trackers.bugzilla.dialog.config.field.label.bugzillaUrl");

    private static final String TITLE_NAME_REPEATED_DIALOG =
            Constant.messages.getString(
                    "bugtracker.trackers.bugzilla.dialog.config.warning.name.repeated.title");
    private static final String TEXT_NAME_REPEATED_DIALOG =
            Constant.messages.getString(
                    "bugtracker.trackers.bugzilla.dialog.config.warning.name.repeated.text");
            ...
            bugzillaUrlTextField
                    ...
                    .addDocumentListener(
                            new DocumentListener() {
                                ...
                                private void checkAndEnableConfirmButton() {
                                    setConfirmButtonEnabled(
                                            getBugzillaUrlTextField().getDocument().getLength()
                                                    > 0);
                                }
                            });
        }
