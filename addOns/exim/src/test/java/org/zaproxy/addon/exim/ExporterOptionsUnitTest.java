...
import org.zaproxy.addon.exim.ExporterOptions.Format;
...
class ExporterOptionsUnitTest extends TestUtils {
    ...
    void shouldBuildWithJustOutputFile() {
        ...
        assertThat(options.getFormat(), is(equalTo(Format.HISTORY)));
    ...
    void shouldSetFormat(Format format) {
        // Given
        ExporterOptions.Builder builder = builderWithOutputFile();
        // When
        ExporterOptions options = builder.setFormat(format).build();
        // Then
        assertThat(options.getFormat(), is(equalTo(format)));
    }

    @Test
    void shouldThrowSettingNullFormat() {
        // Given
        Builder builder = ExporterOptions.builder();
        // When / Then
        Exception ex = assertThrows(IllegalArgumentException.class, () -> builder.setFormat(null));
    ...
    /** Unit test for {@link ExporterOptions.Format}. */
    @Nested
    class FormatUnitTest {

        @ParameterizedTest
        @EnumSource(value = Format.class)
        void shouldHaveToStringRepresentation(Format type) {
            assertThat(type.toString(), is(notNullValue()));
        }

        @ParameterizedTest
        @CsvSource({"HISTORY, history", "SITES, sites"})
        void shouldReturnId(Format type, String expectedId) {
            assertThat(type.getId(), is(equalTo(expectedId)));
        }
        ...
        void shouldConvertFromString(String value, Format expectedFormat) {
            assertThat(Format.fromString(value), is(equalTo(expectedFormat)));
        }
    }
}
