...
public class GspmLegacyImporter {
     ...
     *
     * <p>Per-rule names are read from each plugin entry's {@code <name>} element when present (some
     * bundled policy files, e.g. from the {@code scanpolicies} add-on, include it); left {@code
     * null} otherwise.
     *
     ...
     */
    public static GspmPolicy importPolicy(File file) throws Exception {
        ...
        GspmRuleSet ascanDefault =
                policy.findOrCreateCategoryRuleSet(GspmRuleSet.ALL_CATEGORY + ".ascan");
        ...
        if (pluginsNode != null) {
            ...
            for (int i = 0; i < children.getLength(); i++) {
                ...
                group.addRule(new GspmRuleRef(ruleId, getChildText(child, "name")));
            }
        }
