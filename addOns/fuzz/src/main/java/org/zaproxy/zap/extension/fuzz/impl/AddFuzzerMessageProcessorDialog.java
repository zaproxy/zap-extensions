...
public class AddFuzzerMessageProcessorDialog<T1 extends Message, T2 extends FuzzerMessageProcessor<T1>> extends
        AbstractFormDialog {

    private static final long serialVersionUID = 4460797449668634319L;

    private static final String DIALOG_TITLE = Constant.messages.getString("fuzz.fuzzer.dialog.add.messageprocessor.title");

    private static final String CONFIRM_BUTTON_LABEL = Constant.messages.getString("fuzz.fuzzer.dialog.add.messageprocessor.button.confirm");

    private static final String TYPE_FIELD_LABEL = Constant.messages.getString("fuzz.fuzzer.dialog.add.messageprocessor.label.type");
    ...
    public AddFuzzerMessageProcessorDialog(Dialog owner, FuzzerMessageProcessors<T1, T2> fuzzerMessageProcessors, T1 message) {
        ...
        for (String payloadType : fuzzerMessageProcessors.getFuzzerMessageProcessorUIHandlersNames()) {
            contentsPanel.add(fuzzerMessageProcessors.getPanel(payloadType).getComponent(), payloadType);
        }

        getMessageProcessorUIHandlersComboBox().setSelectedItem(fuzzerMessageProcessors.getDefaultPanelName());
