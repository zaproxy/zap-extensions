...
public class ProcessorsPayloadDialog extends AbstractFormDialog {
    ...
    private static final String CONFIRM_BUTTON_LABEL =
            Constant.messages.getString(
                    "fuzz.fuzzer.dialog.payloads.dialog.processors.button.confirm");

    private static final String REMOVE_DIALOG_TITLE =
            Constant.messages.getString(
                    "fuzz.fuzzer.dialog.payloads.dialog.remove.processor.title");
    private static final String REMOVE_DIALOG_TEXT =
            Constant.messages.getString("fuzz.fuzzer.dialog.payloads.dialog.remove.processor.text");

    private static final String REMOVE_DIALOG_CONFIRM_BUTTON_LABEL =
            Constant.messages.getString(
                    "fuzz.fuzzer.dialog.payloads.dialog.remove.processor.button.confirm");
    private static final String REMOVE_DIALOG_CANCEL_BUTTON_LABEL =
            Constant.messages.getString(
                    "fuzz.fuzzer.dialog.payloads.dialog.remove.processor.button.cancel");

    private static final String REMOVE_DIALOG_CHECKBOX_LABEL =
            Constant.messages.getString(
                    "fuzz.fuzzer.dialog.payloads.dialog.remove.processor.checkbox.label");
    ...
    public ProcessorsPayloadDialog(Window parent, PayloadProcessorsContainer processorsUIHandlers) {
        super(
                parent,
                Constant.messages.getString("fuzz.fuzzer.dialog.payloads.dialog.processors.title"),
                false);
    ...
    protected void clearFields() {
        ...
        payloads = null;
        processorsTablePanel.setProcessors(Collections.emptyList());
    }
    ...
    private class ProcessorsTablePanel
        ...
        public PayloadProcessorTableEntry showModifyDialogue(PayloadProcessorTableEntry e) {
            PayloadProcessorUI processorUI = showModifyDialogueImpl(e, e.getPayloadProcessorUI());
        ...
        private PayloadProcessorUI showModifyDialogueImpl(
                PayloadProcessorTableEntry e, PayloadProcessorUI payloadGeneratorUI) {
            ModifyProcessorDialog modifyProcessorDialog =
    ...
    private ResettableAutoCloseableIterator<Payload> getProcessedPayloads(int numberOfProcessors) {
        List<PayloadProcessor> currentProcessors = new ArrayList<>();
        int count = 0;
        for (PayloadProcessorTableEntry processorEntry : processorsTablePanel.getProcessors()) {
            ...
            currentProcessors.add(processorEntry.getPayloadProcessorUI().getPayloadProcessor());
            count++;
        }
