...
public class ClientTable extends ParosAbstractTable {
    	...
    	try {
			if (!DbUtils.hasTable(conn, TABLE)) {
				...
				PreparedStatement stmt = conn
						.prepareStatement("CREATE CACHED TABLE " + TABLE + " ("
			...
			psInsert = conn.prepareStatement("INSERT INTO " + TABLE + " (client_id, history_id) VALUES (?,?)");

			psUpdate = conn.prepareStatement("UPDATE " + TABLE + " SET history_id = ? WHERE id = ?");
    ...
    public synchronized List<MonitoredPage> list() throws SQLException {
    	...
    	while (rs.next()) {
    		list.add(new MonitoredPage(rs.getLong("id"), rs.getString("client_id"), rs.getInt("history_id")));
    	}
    	rs.close();
