...
class CrossSiteScriptingScanRuleUnitTest extends ActiveScannerTest<CrossSiteScriptingScanRule> {
    ...
    void shouldReportXssInAttributeScriptTagWhenTargetContextIsNullAtLowThreshold()
        ...
        this.nano.addHandler(createAttributeScriptTagHandler(test));
    ...
    void shouldReportXssWeaknessInJsonResponseWithFilteredScript()
        ...
        this.nano.addHandler(createJsonFilteredScriptHandler(test));
    ...
    void shouldReportXssViaDirectAttackWhenEyeCatcherNotReflectedAtMediumThreshold()
        ...
        this.nano.addHandler(createDirectAttackNoEyeCatcherHandler(test));
    ...
    void shouldReportXssOutsideTagsWhenInitialReflectionIsOutsideTagsAtMediumThreshold()
        ...
        this.nano.addHandler(createOutsideTagsInitialOutsideHandler(test));
    ...
    void shouldReportXssViaHeaderSplittingWhenEyeCatcherReflectedInHeaderAtMediumThreshold()
        ...
        this.nano.addHandler(createHeaderSplittingViaHeaderReflectionHandler(test));
    ...
    private NanoServerHandler createAttributeScriptTagHandler(String path) {
    ...
    private NanoServerHandler createJsonFilteredScriptHandler(String path) {
        return new NanoServerHandler(path) {
            @Override
            protected Response serve(IHTTPSession session) {
                String name = getFirstParamValue(session, "name");
    ...
    private NanoServerHandler createDirectAttackNoEyeCatcherHandler(String path) {
        return new NanoServerHandler(path) {
            @Override
            protected Response serve(IHTTPSession session) {
                String name = getFirstParamValue(session, "name");
                ...
                // Ensure the initial discovery phase finds no HtmlContexts (no eye-catcher in
                // body), but allow subsequent direct-attack payloads to be reflected.
    ...
    private NanoServerHandler createOutsideTagsInitialOutsideHandler(String path) {
        return new NanoServerHandler(path) {
            @Override
            protected Response serve(IHTTPSession session) {
                String name = getFirstParamValue(session, "name");
                ...
                // Make the initial eye-catcher reflection be outside of any tags (no DOM element),
                ...
                // For the actual outside-tags payload attempts, reflect the payload inside HTML
    ...
    private NanoServerHandler createHeaderSplittingViaHeaderReflectionHandler(String path) {
        return new NanoServerHandler(path) {
            @Override
            protected Response serve(IHTTPSession session) {
                String name = getFirstParamValue(session, "name");
                ...
                String eyeCatcher = Constant.getEyeCatcher();
                if (name.contains(eyeCatcher)) {
                    // Trigger attackHeader() by reflecting the eye-catcher in a response header,
                ...
                // Avoid triggering other attacks (like performDirectAttack) by only reflecting
                // values that include CR/LF (header splitting payloads).
                if (name.indexOf('\r') == -1 && name.indexOf('\n') == -1) {
                    return newFixedLengthResponse(getHtml("NoInput.html"));
                }

                // Ensure deterministic behavior: only reflect the first header splitting payload
                // (GENERIC_SCRIPT_ALERT) so the alert is raised with that exact attack string.
                if (!name.equals(
                        "test\n\r\n\r" + CrossSiteScriptingScanRule.GENERIC_SCRIPT_ALERT)) {
                    return newFixedLengthResponse(getHtml("NoInput.html"));
                }

                // For header-splitting attacks, reflect the (decoded) value in HTML so the evidence
