...
function authenticate(helper, paramsValues, credentials) {
  ...
  var requestBody =
    "log=" + encodeURIComponent(credentials.getParam("Username"));
  requestBody =
    requestBody +
    "&pwd=" +
    encodeURIComponent(credentials.getParam("Password"));
  requestBody =
    requestBody + "&rememberme=forever&wp-submit=Log+In&testcookie=1";
  ...
  for (var iterator = cookies.iterator(); iterator.hasNext(); ) {
    ...
    print(
      "Manually adding cookie: " + cookie.getName() + " = " + cookie.getValue(),
    );
    state.addCookie(
      new Cookie(
        domain,
