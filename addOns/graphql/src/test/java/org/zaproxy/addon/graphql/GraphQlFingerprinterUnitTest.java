...
class GraphQlFingerprinterUnitTest extends TestUtils {
    ...
    void shouldFingerprintValidData(String graphqlImpl, String response) throws Exception {
        ...
        var fp = new GraphQlFingerprinter(UrlBuilder.build(endpointUrl));
        DiscoveredGraphQlEngine[] discoveredEngine = new DiscoveredGraphQlEngine[1];
        GraphQlFingerprinter.addEngineHandler(
                (engine) -> {
                    discoveredEngine[0] = engine;
                });
        // When
        fp.fingerprint();
        // Then
        Alert.Builder builder = GraphQlFingerprinter.createFingerprintingAlert(discoveredEngine[0]);
        ...
        assertThat((discoveredEngine[0]).getUri().toString(), is(equalTo(endpointUrl)));
        assertThat(graphqlImpl, is(equalTo((discoveredEngine[0]).getName())));
    }
