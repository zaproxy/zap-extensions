...
public class OAuth2AuthenticationMethodType extends AuthenticationMethodType {
    ...
    /**
     * Key used to store the access token captured automatically from the latest successful grant
     * response, so it's available to build introspection poll requests ({@link
     * OAuth2AuthenticationMethod#replaceUserDataInPollRequest}) without any user configuration.
     */
    private static final String CREDENTIAL_PARAM_ACCESS_TOKEN = "accessToken";
    ...
    private static final String CONTEXT_CONFIG_AUTH_OAUTH2_VERIFICATION_URL =
            CONTEXT_CONFIG_AUTH_OAUTH2 + ".verificationurl";
    ...
    private static final String PARAM_VERIFICATION_URL = "verificationUrl";
    ...
    public class OAuth2AuthenticationMethod extends AuthenticationMethod {
        ...
        public OAuth2AuthenticationMethod(OAuth2AuthenticationMethod method) {
            ...
            this.verificationUrl = method.verificationUrl;
        }
        ...
        public WebSession authenticate(
            ...
            try (AuthenticationDiagnostics diags =
                ...
                if (StringUtils.isNotEmpty(verificationUrl)) {
        ...
        private HttpMessage attemptGrant(
            ...
            diags.recordStep(
                    Constant.messages.getString("authhelper.auth.method.diags.steps.authmessage"));
            return msg;
        }
        ...
        /**
         * Captures the {@code access_token}/{@code refresh_token} from a successful token response
         * (if present) and stores them internally so they can be reused - the refresh token on
         * subsequent authentications (kept up to date across rotations), and the access token to
         * build introspection poll requests ({@link #replaceUserDataInPollRequest}) - without any
         * user configuration.
         */
        private void captureTokens(HttpMessage msg, GenericAuthenticationCredentials cred) {
            try {
                JSONObject json = JSONObject.fromObject(msg.getResponseBody().toString());
                captureIfPresent(json, "access_token", cred, CREDENTIAL_PARAM_ACCESS_TOKEN);
        ...
        /**
         * Applies this method's configured client authentication to a token/introspection request:
         * for {@link #CLIENT_AUTH_POST}/{@link #CLIENT_AUTH_NONE} this adds {@code
         * client_id}/{@code client_secret} to {@code bodyParams}; otherwise (the default, {@link
         * #CLIENT_AUTH_BASIC}) it returns the {@code Authorization: Basic ...} header value to set
                    ...
                    + Base64.getEncoder()
                            .encodeToString(
                                    (URLEncoder.encode(clientId, StandardCharsets.UTF_8)
                                                    + ":"
                                                    + URLEncoder.encode(
                                                            StringUtils.defaultString(clientSecret),
        ...
        public ApiResponse getApiResponseRepresentation() {
            ...
            values.put(PARAM_VERIFICATION_URL, verificationUrl);
            return new AuthMethodApiResponseRepresentation<>(values);
        }

        /**
         * When {@link #verificationUrl} is configured and the poll request targets it, builds an
         * RFC 7662 Token Introspection request (POSTing {@code token=<access token>}, with this
         * method's own client authentication) instead of the default behaviour of attaching the
         * session's bearer token via session management - introspection authenticates as the OAuth2
         * client, not as the resource owner, so the two would otherwise collide.
         */
        @Override
        public void replaceUserDataInPollRequest(HttpMessage msg, User user) {
            if (StringUtils.isEmpty(verificationUrl)
                    || !verificationUrl.equals(msg.getRequestHeader().getURI().toString())
            ...
            bodyParams.put(
                    "token",
                    StringUtils.defaultString(cred.getParam(CREDENTIAL_PARAM_ACCESS_TOKEN)));
            String authHeaderValue = applyClientAuthentication(bodyParams);
    ...
    public AuthenticationMethod loadMethodFromSession(Session session, int contextId)
        ...
        method.setVerificationUrl(map.get(PARAM_VERIFICATION_URL));
    ...
    public void persistMethodToSession(
        ...
        map.put(PARAM_VERIFICATION_URL, method.verificationUrl);
    ...
    public void exportData(Configuration config, AuthenticationMethod authMethod) {
        ...
        config.setProperty(CONTEXT_CONFIG_AUTH_OAUTH2_VERIFICATION_URL, method.verificationUrl);
    ...
    public void importData(Configuration config, AuthenticationMethod authMethod)
        ...
        try {
            ...
            method.setVerificationUrl(
                    config.getString(CONTEXT_CONFIG_AUTH_OAUTH2_VERIFICATION_URL));
        } catch (Exception e) {
        ...
        return new ApiDynamicActionImplementor(
            ...
            public void handleAction(JSONObject params) throws ApiException {
                ...
                try {
                    ...
                    method.setVerificationUrl(
                            ApiUtils.getOptionalStringParam(params, PARAM_VERIFICATION_URL));
    ...
    protected class OAuth2AuthenticationMethodOptionsPanel
        ...
        private ZapTextField verificationUrlField;
        ...
        public OAuth2AuthenticationMethodOptionsPanel() {
            ...
            verificationUrlField = new ZapTextField();
            addRow(
                    "authhelper.auth.method.oauth2.label.verificationUrl",
        ...
        public void saveMethod() {
            ...
            getMethod().setVerificationUrl(verificationUrlField.getText());
        ...
        public void bindMethod(AuthenticationMethod method)
            ...
            verificationUrlField.setText(authenticationMethod.getVerificationUrl());
