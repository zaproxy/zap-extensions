...
public class LlmResponseHandler implements ChatModelListener {
    ...
    public void onRequest(ChatModelRequestContext requestContext) {
        output("--- Request ---", requestContext.chatRequest().messages().get(0).toString());
    }

    @Override
    public void onResponse(ChatModelResponseContext responseContext) {
        LOGGER.info("Token usage = {} ", responseContext.chatResponse().tokenUsage());
        output("--- Response ---", responseContext.chatResponse().aiMessage().text());
    }

    @Override
    public void onError(ChatModelErrorContext errorContext) {
        LOGGER.error("LLM Error : {} ", errorContext.error().getMessage());
        output("--- Error ---", errorContext.error().getMessage());
