...
public class GitIndexEntryCache {
		...
		private Map <URI, Map <URI, String>> gitIndexMap = Collections.synchronizedMap (new LRUMap (20)); //max: 20 Git index files (LRU)		
		...
		public synchronized void putIndexEntry (URI gitIndexUri, URI gitIndexEntryUri, String gitSHA1) {
			...
			if (gitIndexMap.containsKey(gitIndexUri)) { 
				...
				indexEntryMap = Collections.synchronizedMap (new LRUMap (1000)); //max: 1000 Git index entries (LRU)
			}
