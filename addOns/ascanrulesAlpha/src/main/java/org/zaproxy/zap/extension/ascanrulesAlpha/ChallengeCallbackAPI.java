...
public abstract class ChallengeCallbackAPI implements CallbackImplementor {
    ...
    public void handleCallBack(HttpMessage msg) {
        ...
        try {
            ...
            if (rcback != null) {
                rcback.getPlugin().notifyCallback(challenge, rcback.getAttackMessage(), msg, rcback.getContext());
     ...
     * @param attack
     */
    public void registerCallback(String challenge, ChallengeCallbackPlugin plugin, HttpMessage attack, Object context) {
        // Maybe we'va a lot of dirty entries
        cleanExpiredCallbacks();

        // Already synchronized (no need for a monitor)
        regCallbacks.put(challenge, new RegisteredCallback(plugin, attack, context));
    }
    ...
    private static class RegisteredCallback {
        ...
        public RegisteredCallback(ChallengeCallbackPlugin plugin, HttpMessage msg, Object context) {
            this.plugin = plugin;
