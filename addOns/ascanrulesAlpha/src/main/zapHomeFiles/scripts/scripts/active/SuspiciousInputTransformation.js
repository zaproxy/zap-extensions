...
references: []  # Added dynamically based on the specific transformation detected
category: injection
risk: high
confidence: medium
...
const inputTransformationChecks = [
  ...
  },
  {
    ...
    references: ["https://cwe.mitre.org/data/definitions/172.html"],
  },
  {
  ...
  },
  {
    ...
    references: ["https://www.unicode.org/charts/case/index.html"],
  },
  {
    ...
    references: ["https://codepoints.net/combining_diacritical_marks?lang=en"],
  },
];
      ...
      for (let j = 0; j < expectedValues.length; j++) {
        ...
        if (
          ...
          if (attempt === CHECK_CONFIRM_COUNT - 1) {
            ...
            as.newAlert()
              .setName(`Suspicious Input Transformation: ${check.name}`)
              .setAlertRef(check.alertRef)
              .setParam(param)
              .setAttack(attackPayload)
              .setEvidence(expectedValue)
              .setMessage(testMsg)
              .setReference(check.references.join("\n"))
              .raise();
