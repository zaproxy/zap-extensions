...
id: ${SCAN_RULE_ID}
name: Swagger Secret & Vulnerability Detector
description: >
...
function scanHost(as, msg) {
  ...
  // Return if currentPath was already checked in pass 1
  if (SWAGGER_PATHS.includes(currentPath)) {
    return;
  }
  ...
  for (let r = 0; r < SWAGGER_REGEX_PATHS.length; r++) {
    if (as.isStop()) return; // ← inserted check
    if (
      SWAGGER_REGEX_PATHS[r].test(currentPath) &&
...
function scanPath(as, origMsg, scheme, host, port, pathOnly, fullPath) {
  ...
  alertRaised = detectSecrets(as, requestMsg, fullPath, body) || alertRaised;
  return alertRaised;
}
