...
/** Unit test for {@link PassiveScannerOptions}. */
class PassiveScannerOptionsUnitTest {
    ...
    void shouldHaveConfigVersionKey() {
        assertThat(options.getConfigVersionKey(), is(equalTo("pscans[@version]")));
    }
    ...
    void shouldLoadThreadsFromConfig(int threads) {
        ...
        options.load(configuration);
        // Then
        assertThat(options.getPassiveScanThreads(), is(equalTo(threads)));
    }
    ...
    void shouldDefaultThreads() {
        ...
        // Then
        assertThat(
                options.getPassiveScanThreads(), is(equalTo(Constant.getDefaultThreadCount() / 2)));
    }
}
