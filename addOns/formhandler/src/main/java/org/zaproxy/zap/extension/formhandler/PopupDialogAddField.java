...
class PopupDialogAddField extends DialogAddField {
	...
	private ExtensionFormHandler extFormHandler = Control.getSingleton().getExtensionLoader()
	...
	protected boolean validateFields() {
		...
		for (String field : Control.getSingleton().getExtensionLoader().getExtension(ExtensionFormHandler.class)
	...
	/**
	 * When the Add button is clicked, create a new field. This field will be
	 * created with the name, value and enabled input by the user. The name will
	 * always be lower case.
	 */
	@Override
	protected void performAction() {
		extFormHandler.addFormHandlerFieldName(getNameTextField().getText().toLowerCase(),
