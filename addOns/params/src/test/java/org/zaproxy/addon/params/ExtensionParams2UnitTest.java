...
/** Unit test for {@link ExtensionParams2}. */
class ExtensionParams2UnitTest extends TestUtils {
    ...
    @Test
    void shouldHaveName() {
        assertThat(extension.getName(), is(equalTo("ExtensionParams2")));
    }
    ...
    void shouldNotHookWhenCoreParamsExtensionEnabled() {
        ...
        extension.hook(extensionHook);
        // Then
        verify(extensionHook, never()).addSessionListener(any());
        verify(extensionHook, never()).addApiImplementor(any());
        verify(extensionHook, never()).addSiteMapListener(any());
    }
}
