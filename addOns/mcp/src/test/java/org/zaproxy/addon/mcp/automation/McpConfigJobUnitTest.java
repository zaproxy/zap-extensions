...
class McpConfigJobUnitTest extends TestUtils {
    ...
    void shouldReturnCorrectDefaults() {
        ...
        // When / Then
        assertThat(job.getType(), is(equalTo("mcp-config")));
    ...
    void shouldApplyParams() {
        ...
        assertThat(mcpParam.isSecurityKeyEnabled(), is(true));
    }

    @Test
    void shouldApplyEmptySecurityKey() {
        // Given
        McpConfigJob job = new McpConfigJob(extMcp);
        job.getParameters().setSecurityKey("");

        // When
        job.runJob(new AutomationPlan().getEnv(), new AutomationProgress());

        // Then
        assertThat(mcpParam.getSecurityKey(), is(equalTo("")));
