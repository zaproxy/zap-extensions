...
public class ZapStartActiveScanTool extends ZapStartScanTool {
    ...
    @Override
    protected String getJobFinishedWithoutStartingMessage() {
        return "active scan job finished without starting";
    }
    ...
    protected void configureJob(AutomationJob job, JsonNode arguments) {
        if (arguments == null || !arguments.has("policy") || arguments.get("policy").isNull()) {
            return;
        }
        String policy = arguments.get("policy").asText().trim();
        if (policy.isEmpty()) {
            return;
        }
        if (job instanceof ActiveScanJob ascJob) {
            ascJob.getParameters().setPolicy(policy);
        }
    }
}
