...
public class ImportMcpServerDialog extends AbstractDialog {
    ...
    private JPasswordField fieldSecurityKey;
    ...
    private JPasswordField getSecurityKeyField() {
        if (fieldSecurityKey == null) {
            fieldSecurityKey = new JPasswordField(30);
        }
    ...
    private void doImport() {
        ...
        if (url.isEmpty()) {
            View.getSingleton()
                    .showWarningDialog(
                            this,
                            Constant.messages.getString("mcp.importserver.dialog.error.emptyurl"));
        ...
        try {
            ...
            View.getSingleton()
                    .showWarningDialog(
                            this,
                            Constant.messages.getString(
                                    "mcp.importserver.dialog.error.invalidurl", url));
            getServerUrlField().requestFocusInWindow();
            return;
        }

        char[] key = getSecurityKeyField().getPassword();
        String securityKey = key == null || key.length == 0 ? null : new String(key);
