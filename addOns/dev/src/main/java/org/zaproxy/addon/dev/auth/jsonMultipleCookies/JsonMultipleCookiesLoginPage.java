...
public class JsonMultipleCookiesLoginPage extends TestPage {

    private static final Logger LOGGER = LogManager.getLogger(JsonMultipleCookiesLoginPage.class);

    public JsonMultipleCookiesLoginPage(TestProxyServer server) {
        super(server, "login");
    }
    ...
    public void handleMessage(HttpMessageHandlerContext ctx, HttpMessage msg) {
        ...
        if (getParent().isValid(username, password)) {
            token = getParent().getTempToken(username);
        ...
        if (token != null) {
            msg.getResponseHeader()
                    .addHeader(HttpHeader.SET_COOKIE, "temp=" + token + "; SameSite=Strict");
        }
    }
