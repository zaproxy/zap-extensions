 ...
 *       consent), {@code POST /token} (all 4 grant types: {@code authorization_code} with optional
 *       PKCE, {@code client_credentials}, {@code password}, {@code refresh_token}), and {@code POST
 *       /introspect} (RFC 7662 token introspection, for verification).
...
public class OAuth2RootDir extends TestAuthDirectory {
    ...
    private static final String REDIRECT_URI = "https://app.oauth2.zap/callback.html";

    private static final Map<String, String> REGISTERED_REDIRECT_URIS =
            Map.of(CONFIDENTIAL_CLIENT_ID, REDIRECT_URI, PUBLIC_CLIENT_ID, REDIRECT_URI);
    ...
    private void handleAuthorize(HttpMessage msg) throws Exception {
        ...
        String registeredRedirectUri = REGISTERED_REDIRECT_URIS.get(clientId);
        if (registeredRedirectUri == null || !registeredRedirectUri.equals(redirectUri)) {
            setErrorResponse(msg, "Unknown client_id or unregistered redirect_uri");
            return;
        }
    ...
    /**
     * RFC 7662 OAuth 2.0 Token Introspection - lets a client (authenticated the same way as at the
     * token endpoint) check whether a token is still valid, for use as a verification/poll target.
     */
    JSONObject handleIntrospect(HttpMessage msg) throws Exception {
        String[] clientCredentials = extractClientCredentials(msg);
        String clientId = clientCredentials[0];
        String clientSecret = clientCredentials[1];
    ...
    JSONObject handleRefreshTokenGrant(HttpMessage msg, String clientId, String clientSecret) {
        if (!isValidClient(clientId, clientSecret)) {
            return errorJson("invalid_client", "Client authentication failed");
        }
        String refreshToken = DevUtils.getFormParam(msg, "refresh_token");
        String username = refreshToken == null ? null : refreshTokenToUser.remove(refreshToken);
        if (username == null) {
    ...
    JSONObject issueTokens(String subject, String scope, boolean includeRefreshToken) {
        JSONObject response = new JSONObject();
    ...
    static boolean matchesCodeChallenge(
        ...
        String method = StringUtils.isEmpty(codeChallengeMethod) ? "plain" : codeChallengeMethod;
