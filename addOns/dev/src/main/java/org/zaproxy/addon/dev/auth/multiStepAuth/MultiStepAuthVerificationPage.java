...
/**
 * Session verification endpoint. Returns HTTP 200 + {"result":"OK","user":"..."} for a valid token,
 * or HTTP 403 + {"result":"FAIL"} otherwise — the 403 lets ZAP detect the unauthenticated state by
 * status code.
 */
public class MultiStepAuthVerificationPage extends TestPage {

    private static final Logger LOGGER = LogManager.getLogger(MultiStepAuthVerificationPage.class);

    public MultiStepAuthVerificationPage(TestProxyServer server) {
        super(server, "user");
    }
