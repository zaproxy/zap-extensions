		...
		try {
			...
			for (int i = 0; i < numberOfRequests; i++) {
				...
				while ( HttpStatusCode.isRedirection(msgCpy.getResponseHeader().getStatusCode())) {
					...
					if ( this.debugEnabled ) log.debug("Following redirect "+redirectCount + " for message "+ i + " of " +numberOfRequests + " iterations of the original query");
				...
				if ( ! HttpStatusCode.isSuccess(msgCpy.getResponseHeader().getStatusCode())) {
					log.warn("The original URL ["+getBaseMsg().getRequestHeader().getURI()+"] returned a non-OK HTTP status "+msgCpy.getResponseHeader().getStatusCode()+" (after "+i + " of "+ numberOfRequests+" steps). Could be indicative of SQL Injection, or some other error. The URL is not stable enough to look at Username Enumeration" );
			...
			for (Iterator<HtmlParameter> iter = htmlParams.iterator(); iter.hasNext(); ) {
				...
				for (int i = 0; i < numberOfRequests && continueForParameter; i++) {
					...
					while ( HttpStatusCode.isRedirection(msgCpy.getResponseHeader().getStatusCode())) {
						...
						if ( this.debugEnabled ) log.debug("Following redirect "+redirectCount + " for message "+ i + " of " +numberOfRequests + " iterations of the modified query");
				...
				if (continueForParameter==true)	{
					...
					if (! longestCommonSubstringA.equals(longestCommonSubstringB)) {
						...
						String extraInfo = Constant.messages.getString("ascanbeta.usernameenumeration.alert.extrainfo", 
						...
						String attack = Constant.messages.getString("ascanbeta.usernameenumeration.alert.attack", currentHtmlParameter.getType(), currentHtmlParameter.getName());
