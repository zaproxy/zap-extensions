...
public class SessionMonitoredClientsPanel extends AbstractParamPanel {
	...
	private JPanel getPanelSession() {
		if (panelSession == null) {
	        ...
	        panelSession.add(this.getAllInScope(), 
	        		LayoutHelper.getGBC(0, 0, 1, 1.0, 0.0));
			
	        panelSession.add(new JLabel(Constant.messages.getString("plugnhack.session.label.include")), 
	        		LayoutHelper.getGBC(0, 1, 1, 0.0D, new Insets(10,0,5,0)));
	        panelSession.add(getIncludeScrollPane(), 
	        		LayoutHelper.getGBC(0, 2, 1, 1.0, 1.0, GridBagConstraints.BOTH, null));

	        panelSession.add(new JLabel(Constant.messages.getString("plugnhack.session.label.exclude")), 
	        		LayoutHelper.getGBC(0, 3, 1, 0.0D, new Insets(10,0,5,0)));
	        panelSession.add(getExcludeScrollPane(), 
	...
	private JCheckBox getAllInScope() {
		if (allInScope == null) {
			allInScope = new JCheckBox(Constant.messages.getString("plugnhack.session.label.inscope"));
		}
	...
	private SingleColumnTableModel getIncludeModel() {
		if (includeModel == null) {
			includeModel = new SingleColumnTableModel(Constant.messages.getString("plugnhack.session.table.header.include"));
		}
		return includeModel;
	}
	...
	private SingleColumnTableModel getExcludeModel() {
		if (excludeModel == null) {
			excludeModel = new SingleColumnTableModel(Constant.messages.getString("plugnhack.session.table.header.exclude"));
		}
		return excludeModel;
	}
	...
	public void refresh() {
		...
		this.includeCache = (ArrayList<String>) ((ArrayList<String>)this.includeModel.getLines()).clone();
		this.excludeCache = (ArrayList<String>) ((ArrayList<String>)this.excludeModel.getLines()).clone();
	}
}
