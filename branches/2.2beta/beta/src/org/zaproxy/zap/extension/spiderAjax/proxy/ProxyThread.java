...
// ZAP: 2013/12/13 Changed to allow to overwrite a message and check if a request is a SSL/TLS handshake.

package org.zaproxy.zap.extension.spiderAjax.proxy;
...
//	private static ArrayList 		processForwardList = new ArrayList();
	...
	public ProxyThread(ProxyServer server, Socket socket) {
		parentServer = server;
	...
	public void run() {
		...
		try {
			BufferedInputStream bufferedInputStream = new BufferedInputStream(inSocket.getInputStream());
...
//            System.out.println("send required: " + msg.getRequestHeader().getURI().toString());
			...
			synchronized (semaphore) {
			    ...
			    if (notifyOverwriteListenersRequestSend(msg)) {
			        send = false;
...
//					getHttpSender().sendAndReceive(msg, httpOut, buffer);
			        if (send) {
			            ...
			            if (!notifyOverwriteListenersResponseReceived(msg)) {
...
//			    	System.out.println("HttpException");
	...
	private boolean notifyOverwriteListenersRequestSend(HttpMessage httpMessage) {
		for (OverwriteMessageProxyListener listener : parentServer.getOverwriteMessageProxyListeners()) {
			try {
	...
	private boolean notifyOverwriteListenersResponseReceived(HttpMessage httpMessage) {
		for (OverwriteMessageProxyListener listener : parentServer.getOverwriteMessageProxyListeners()) {
			try {
