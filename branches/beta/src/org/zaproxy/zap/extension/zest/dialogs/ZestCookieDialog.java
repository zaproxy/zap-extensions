...
public class ZestCookieDialog extends StandardFieldsDialog implements ZestDialog {
	...
	public void init (ZestScriptWrapper script, String domain, String name, String value, String path, 
			boolean add, int index, boolean canBeEmpty) {
		this.script = script;
		this.add = add;
		this.index = index;
		if (add) {
	...
	public void save() {
		if (add) {
			this.model.add(this.getStringValue(FIELD_PARAM_DOMAIN), this.getStringValue(FIELD_PARAM_NAME), 
					this.getStringValue(FIELD_PARAM_VALUE), this.getStringValue(FIELD_PARAM_PATH));
		} else {
			this.model.replace(this.index, this.getStringValue(FIELD_PARAM_DOMAIN), this.getStringValue(FIELD_PARAM_NAME), 
					this.getStringValue(FIELD_PARAM_VALUE), this.getStringValue(FIELD_PARAM_PATH));
		}
	}
