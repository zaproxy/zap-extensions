...
public class ZestZapUtils {
	...
	public static String toUiString(ZestElement za, boolean incParams, boolean isShadow) {
		if (za instanceof ZestScript) {
			...
			return MessageFormat.format(
					...
					toUiString((ZestElement)zas.getRootExpression(), incParams, isShadow));
			
		} else if (za instanceof ZestConditional) {
			ZestConditional zac = (ZestConditional) za;
			if (isShadow) {
				return Constant.messages.getString("zest.element.conditional.else");
			} else {
				return MessageFormat.format(
						Constant.messages.getString("zest.element.conditional.if"), 
						toUiString((ZestElement)zac.getRootExpression(), incParams, isShadow));
			}
			...
			if (incParams) {
				return MessageFormat.format(
						Constant.messages.getString("zest.element.expression.statuscode"), sca.getCode());
			} else {
				return Constant.messages.getString("zest.element.expression.statuscode.title");
			}
		} else if (za instanceof ZestExpressionLength) {
			ZestExpressionLength sla = (ZestExpressionLength) za;
			if (incParams) {
				return MessageFormat.format(
						Constant.messages.getString("zest.element.expression.length"), sla.getLength(), sla.getApprox());
			} else {
				return Constant.messages.getString("zest.element.expression.length.title");
			}
			...
			if (incParams) {
				if (zhr.isGreaterThan()) {
					return MessageFormat.format(
							Constant.messages.getString("zest.element.expression.resptimegt"), zhr.getTimeInMs());
				} else {
				return MessageFormat.format(
						Constant.messages.getString("zest.element.expression.resptimelt"), zhr.getTimeInMs());
				}
			} else {
				return Constant.messages.getString("zest.element.expression.resptime.title");
			}
			...
			if (incParams) {
				if (zer.isInverse()) {
					return MessageFormat.format(
							Constant.messages.getString("zest.element.expression.regex.exc"), 
							zer.getLocation(), zer.getRegex());
				} else {
					return MessageFormat.format(
							Constant.messages.getString("zest.element.expression.regex.inc"), 
							zer.getLocation(), zer.getRegex());
				}
			} else {
			...
			if (incParams) {
				return MessageFormat.format(
						Constant.messages.getString("zest.element.expression.url"), incStr, excStr);
			} else {
	...
	public static ZestElement getElement(ScriptNode node) {
		...
		log.debug("getElement " + node.getNodeName() + " Unrecognised class: " + node.getUserObject().getClass().getCanonicalName());
		return null;
	}
