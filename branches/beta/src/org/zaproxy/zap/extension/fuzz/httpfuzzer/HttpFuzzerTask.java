...
public class HttpFuzzerTask extends AbstractFuzzerTask<HttpMessage> {
    ...
    protected void runImpl(HttpMessage message, List<Object> payloads) {
        ...
        HttpFuzzResult result = new HttpFuzzResult(
                getId(),
        ...
        for (int retryCount = 0;;) {
            ...
            try {
                ...
                if (retryCount >= maxRetries) {
                    String retriesInfo = maxRetries <= 0 ? "" : " After retry " + retryCount + " of " + maxRetries + ".";
                    LOGGER.warn("Failed to send a fuzzed message: '" + e.getMessage() + "'." + retriesInfo);
                ...
                if (LOGGER.isDebugEnabled()) {
                    LOGGER.debug("Failed to send HTTP fuzzed message '" + e.getMessage() + "'. Send retry " + retryCount
                            + " of " + maxRetries + ".");
                }
            }
        }
