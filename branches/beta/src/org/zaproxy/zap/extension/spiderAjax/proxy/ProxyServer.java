...
// ZAP: 2013/12/13 Changed to support overwrite of messages.
...
public class ProxyServer implements Runnable {
    ...
    protected Vector<OverwriteMessageProxyListener> overwriteListeners = new Vector<>();
...
//		ServerSocket socket = new ServerSocket(port, 300, InetAddress.getByName(ip)getProxyParam().getProxyIp()));
	...
	public void addOverwriteMessageProxyListener(OverwriteMessageProxyListener listener) {
		overwriteListeners.add(listener);
		Collections.sort(overwriteListeners, getListenersComparator());
	}

	public void removeOverwriteMessageProxyListener(OverwriteMessageProxyListener listener) {
		overwriteListeners.remove(listener);
	}

	public void removeOverwriteMessageProxyListeners() {
		overwriteListeners.clear();
	}

	List<OverwriteMessageProxyListener> getOverwriteMessageProxyListeners() {
		return overwriteListeners;
	}
}
