...
public class EventStreamPanel extends AbstractPanel implements EventStreamObserver {
	 ...
	 */
    public static final int EVENT_STREAM_OBSERVING_ORDER = EventStreamStorage.EVENT_STREAM_OBSERVING_ORDER + 5;
...
//	private WebSocketBreakpointsUiManagerInterface brkManager;
	 ...
	 * @param brkManager 
	 */
	public EventStreamPanel(TableEventStream table) {//, WebSocketBreakpointsUiManagerInterface brkManager) {
		super();
...
//			constraints = new GridBagConstraints();
//			constraints.gridx = x++;
//			panelToolbar.add(new JLabel(Constant.messages.getString("sse.toolbar.stream.label")), constraints);
//			
////			constraints = new GridBagConstraints();
////			constraints.gridx = x++;
...
//	}
	...
	private JButton getOptionsButton() {
		if (optionsButton == null) {
			...
			optionsButton.setIcon(new ImageIcon(EventStreamPanel.class.getResource("/resource/icon/16/041.png")));
			optionsButton.addActionListener(new ActionListener () {
				@Override
				public void actionPerformed(ActionEvent e) {
					Control.getSingleton().getMenuToolsControl().options(
							Constant.messages.getString("sse.panel.title"));
				}
			});
		}
	...
	private Component getFilterButton() {
		if (filterButton == null) {
			...
			filterButton.setIcon(new ImageIcon(EventStreamPanel.class.getResource("/resource/icon/16/054.png")));	// 'filter' icon
	...
	private Component getShowHandshakeButton() {
		if (handshakeButton == null) {
			...
			handshakeButton.setIcon(new ImageIcon(EventStreamPanel.class.getResource("/resource/icon/16/handshake.png")));
			handshakeButton.setToolTipText(Constant.messages.getString("sse.filter.button.handshake"));
...
//			});
	...
	private Component getAddBreakpointButton() {
		if (brkButton == null) {
			...
			brkButton.setIcon(new ImageIcon(EventStreamPanel.class.getResource("/resource/icon/16/break_add.png")));
...
//			});
	...
	/**
	 * Lazy initializes the part of the SSE tab that is used to display
	 * the events.
	 * 
	 * @return
	 */
	...
	@Override
	public void onServerSentEventStateChange(final State state,
			final ServerSentEventStream stream) {
...
//				channelsModel.addElement(stream);
	...
	public EventStreamViewFilterDialog getFilterDialog() {
		if (filterDialog == null) {
			filterDialog = new EventStreamViewFilterDialog(View.getSingleton().getMainFrame(), true);
