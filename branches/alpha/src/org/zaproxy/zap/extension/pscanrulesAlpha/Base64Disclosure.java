...
public class Base64Disclosure extends PluginPassiveScanner {
	...
	/**
	 * a set of patterns used to identify Base64 encoded data. Set a minimum length to reduce false positives.
	 ...
	 * which is used to calculate the probability of this occurring, and eliminate
		...
		for ( Pattern pattern: base64Patterns) {
			...
			for (String haystack: responseparts) {
				...
				while (matcher.find()) {
					...
					try {
						//if the string had the "-_" alphabet, replace the - and _ with + and / respectively
						...
						if (log.isDebugEnabled()) log.debug("["+tempbase64evidence + "] (modified from ["+base64evidence+"]) could not be decoded as Base64 data");
						continue;
					}
