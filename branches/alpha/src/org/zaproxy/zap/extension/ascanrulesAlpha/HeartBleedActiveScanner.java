...
public class HeartBleedActiveScanner extends AbstractHostPlugin {
	...
	/**
	 * Heartbeat Record Type, for both requests and responses.
	 * aka "heartbeat content type" in rfc6520
	 * 0x18 = 24 in decimal 
	 */
	...
	static final byte heartbeatBuffer [] = {
		0x00, 0x03,		// data Length = 0x00 0x03 = 3 in decimal. This length is important later...
		0x01, 			// heartbeat message type. 0x01 = heartbeat request, 0x02 = heartbeat response
		0x40, 0x00		// payload length to be sent back by the server.  0x40 0x00 = 16384 in decimal
			...
			for (int tlsIndex = 0; tlsIndex < tlsBuffers.length; tlsIndex ++) { 
				...
				try {
					...
					try {
						...
						throw new IOException (tlsNames[tlsIndex] + " is not supported by the server, or a common cipher suite could not be agreed");
					}
					...
					if (log.isDebugEnabled()) 
						log.debug("The SSL server does not appear to be vulnerable, using "+tlsNames[tlsIndex]+ ": "+ e.getMessage());
				}
				finally {
					if (log.isDebugEnabled()) 
	 ...
	 * @throws IOException
	 */
	boolean isVulnerable (InputStream is, OutputStream os, int timeoutMs, byte [] tlsVersionBuffer) throws IOException{
		...
		while (true && currentTime <= timeoutTime) {
			...
			if (log.isDebugEnabled()) log.debug("Got a message of type 0x"+ Integer.toHexString(sslRecord.typ)+ " from the server: "+ Hex.encodeHexString(sslRecord.pay));

			if ( sslRecord.typ == heartbeatRecordByte  ) {
				//received the heartbeat response
				if ( sslRecord.len > 3 ) { 
					if (log.isDebugEnabled()) log.debug("VULNERABLE. Got more data back than what we sent in");
					//Got > 3 bytes back. Vulnerable.
					return true;
				}
				else	{
					//Got <=3 bytes back. NOT Vulnerable.
					if (log.isDebugEnabled()) log.debug("NOT VULNERABLE. Got back <=3 bytes. Boo hoo.");
					return false;
				}
			}

			//server returned a fatal alert. we will just ignore warning alerts for now and hope for the best
				...
				if ( sslRecord.pay[0]== 0x02 ) {
					...
					if (log.isDebugEnabled()) { 
						...
						switch (sslRecord.pay[1]) {
							case 0: msg="close_notify"; break;
							case 10: msg="unexpected_message"; break;
							case 20: msg="bad_record_mac"; break;
							case 21: msg="decryption_failed_RESERVED"; break;
							case 22: msg="record_overflow"; break;
							case 30: msg="decompression_failure"; break;
							case 40: msg="handshake_failure"; break;
							case 41: msg="no_certificate_RESERVED"; break;
							case 42: msg="bad_certificate"; break;
							case 43: msg="unsupported_certificate"; break;
							case 44: msg="certificate_revoked"; break;
							case 45: msg="certificate_expired"; break;
							case 46: msg="certificate_unknown"; break;
							case 47: msg="illegal_parameter"; break;
							case 48: msg="unknown_ca"; break;
							case 49: msg="access_denied"; break;
							case 50: msg="decode_error"; break;
							case 51: msg="decrypt_error"; break;
							case 60: msg="export_restriction_RESERVED"; break;
							case 70: msg="protocol_version"; break;
							case 71: msg="insufficient_security"; break;
							case 80: msg="internal_error"; break;
							case 90: msg="user_canceled"; break;
							case 100: msg="no_renegotiation"; break;
							case 110: msg="unsupported_extension"; break;
						}
		...
		if (log.isDebugEnabled()) log.debug("NOT VULNERABLE. No suitable heartbeat response within the timeout");
		return false;
	}
	/**
