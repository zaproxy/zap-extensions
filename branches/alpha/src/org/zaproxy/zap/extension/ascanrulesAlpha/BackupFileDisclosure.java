...
public class BackupFileDisclosure extends AbstractAppPlugin {
	 ...
	 */
	static final Set <String> fileExtensions = new LinkedHashSet<String>(Arrays.asList(new String [] 
			{
			".bak",".backup",".bac",".zip",".tar",".jar", ".log",".swp","~" /* no "." */	,".old"	,".~bk"	,".orig",".tmp", ".exe"
			,".0",".1",".2",".3",".gz",".bz2",".7z",".s7z",".lz",".z",".lzma",".lzo",".apk"
			,".cab",".rar",".war",".ear",".tar.gz",".tgz",".tar.z",".tar.bz2",".tbz2"
			,".tar.lzma",".tlz",".zipx",".iso",".src",".dev",".a",".a",".ar",".cbz",".cpio"
			,".shar",".lbr",".lbr",".mar",".f",".rz",".sfark",".xz",".ace",".afa",".alz"
			,".arc",".arj",".ba",".bh",".cfs",".cpt",".dar",".dd",".dgc",".dmg",".gca"
			,".ha",".hki",".ice",".inc",".j",".kgb",".lhz",".lha",".lzk",".pak",".partimg.",".paq6"
			,".paq7",".paq8",".pea",".pim",".pit",".qda",".rk",".sda",".sea",".sen",".sfx"
			,".sit",".sitx",".sqx","s.xz",".tar.7z",".tar.xz",".uc",".uc0",".uc2",".ucn",".ur2",".ue2",".uca",".uha"
			,".wim",".xar",".xp3",".yz1",".zoo",".zpaq",".zz", ".include"
			...
			,"1","_1","2","_2","x","_x","bak","_bak","old","_old","a","b","c","d","e","f","_a","_b","_c","_d","_e","_f"
			,"inc","_inc", "_backup"
			}));
	 ...
	 */
	static final Set <String> fileSuffixes = new LinkedHashSet<String>(Arrays.asList(new String [] 
			{
			" - Copy"
			," - Copy (2)"
			," - Copy (3)"
	 ...
	 */
	static final Set <String> filePrefixes = new LinkedHashSet<String>(Arrays.asList(new String [] 
			{
			"Copy of "
			,"Copy (2) of "
			,"Copy (3) of "
	...
	public void scan() {
		...
		try {
			...
			if ( filename != null && filename.length() > 0) {
				...
				if (log.isDebugEnabled()) {
					log.debug ("The URI has no filename component, so there is not much point in looking for a corresponding backup file!");
				}				
			}
		} catch (Exception e) {
	...
	private void findBackupFile(HttpMessage originalMessage) throws Exception {
		...
		try {
			...
			String randomfilename=RandomStringUtils.random(filename.length(), "abcdefghijklmoopqrstuvwxyz9123456789");
			...
			HttpMessage nonexistfilemsg= new HttpMessage (new URI(originalURI.getScheme(), originalURI.getAuthority(), randomfilepath, null, null));
			try {
			...
			if ( nonexistfilemsg.getResponseHeader().getStatusCode() != HttpStatus.SC_NOT_FOUND ) {
				gives404s=false;
				if (log.isDebugEnabled()) log.debug("The server does not return a 404 status for a non-existent path: "+ nonexistfilemsg.getRequestHeader().getURI().getURI());
			} else {
				gives404s=true;
				if (log.isDebugEnabled()) log.debug("The server gives a 404 status for a non-existent path: "+ nonexistfilemsg.getRequestHeader().getURI().getURI());
			}
			...
			if (pathbreak.length > 2) {  //the file has a parent folder that is not the root folder (ie, there is a parent folder to mess with)
				...
				String randomparentfoldername=RandomStringUtils.random(parentfoldername.length(), "abcdefghijklmoopqrstuvwxyz9123456789");
				...
				if ( nonexistparentmsg.getResponseHeader().getStatusCode() != HttpStatus.SC_NOT_FOUND ) {
					...
					if (log.isDebugEnabled()) log.debug("The server does not return a 404 status for a non-existent parent path: "+ nonexistparentmsg.getRequestHeader().getURI().getURI());
				} else {
					parentgives404s=true;
					if (log.isDebugEnabled()) log.debug("The server gives a 404 status for a non-existent parent path: "+ nonexistparentmsg.getRequestHeader().getURI().getURI());
				}
			}
			...
			if (positionDirectorySuffixInjection >= 0)
				positionDirectoryPrefixInjection = path.substring(0,positionDirectorySuffixInjection).lastIndexOf("/") +1;
			...
			for (String fileExtensionToTry: fileExtensions) {
				...
				candidateBackupFileURIs.add(new URI(originalURI.getScheme(), originalURI.getAuthority(), candidateBackupFilePath, null, null));
				...
				if (! actualfileExtension.equals("") && doSwitchFileExtension ) {
					candidateBackupFilePath = path.substring(0, positionExtensionInjection) + fileExtensionToTry + actualfileExtension;
			...
			for (String fileSuffixToTry: fileSuffixes ) {
				...
				String candidateBackupFilePath = path.substring(0, positionFileSuffixInjection) + fileSuffixToTry + (positionFileSuffixInjection>=path.length()?"":path.substring(positionFileSuffixInjection));
			...
			for (String filePrefixToTry: filePrefixes ) {
				...
				String candidateBackupFilePath = path.substring(0, positionFilePrefixInjection) + filePrefixToTry + (positionFilePrefixInjection>=path.length()?"":path.substring(positionFilePrefixInjection));
			...
			if (pathbreak.length > 2) {
				...
				for (String fileSuffixToTry: fileSuffixes ) {
					...
					String candidateBackupFilePath = path.substring(0, positionDirectorySuffixInjection) + fileSuffixToTry + (positionDirectorySuffixInjection>=path.length()?"":path.substring(positionDirectorySuffixInjection));
					log.debug("Directory Suffix (insert): '"+ candidateBackupFilePath + "'");
					candidateBackupFileChangedFolderURIs.add(new URI(originalURI.getScheme(), originalURI.getAuthority(), candidateBackupFilePath, null, null));				 
					counted++;
					if (counted > numSuffixesToTry) {
						break; //out of the loop.
					}
				...
				for (String filePrefixToTry: filePrefixes ) {
					...
					String candidateBackupFilePath   = path.substring(0, positionDirectoryPrefixInjection) + filePrefixToTry + (positionDirectoryPrefixInjection>=path.length()?"":path.substring(positionDirectoryPrefixInjection));					
					log.debug("Directory Suffix (insert): '"+ candidateBackupFilePath + "'");
					candidateBackupFileChangedFolderURIs.add(new URI(originalURI.getScheme(), originalURI.getAuthority(), candidateBackupFilePath, null, null));				 
					counted++;
					if (counted > numSuffixesToTry) {
			...
			for (URI candidateBackupFileURI: candidateBackupFileURIs) {
				byte [] disclosedData = {};
				if (log.isDebugEnabled()) log.debug("Trying possible backup file path: "+ candidateBackupFileURI.getURI());						
				...
				if ( 	( gives404s && requestStatusCode != HttpStatus.SC_NOT_FOUND) || 
					...
					bingo(	Alert.RISK_MEDIUM, 
							...
							Constant.messages.getString("ascanalpha.backupfiledisclosure.evidence", originalURI, candidateBackupFileURI.getURI()),
							requestmsg   //originalMessage
							);
				}
				
				if (isStop()) {
					if (log.isDebugEnabled()) log.debug("The scanner was stopped in response to a user request");
					return;
				}
			}
			...
			for (URI candidateBackupFileURI: candidateBackupFileChangedFolderURIs) {
				byte [] disclosedData = {};
				if (log.isDebugEnabled()) log.debug("Trying possible backup file path (with changed parent folder): "+ candidateBackupFileURI.getURI());						
				HttpMessage requestmsg= new HttpMessage (candidateBackupFileURI);
				try {
					requestmsg.setCookieParams(originalMessage.getCookieParams());
					}
