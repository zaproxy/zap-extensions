...
public class ZestActionDialog extends StandardFieldsDialog {
	...
	public void init (ZestNode parent, ZestRequest req, ZestAction action, boolean add) {
		this.add = add;
		this.parent = parent;
		this.request = req;
		this.action = action;
	...
	public void save() {
		...
		if (add) {
			if (request == null) {
				...
				extension.addAfterRequest(parent, request, action);
			}
		} else {
			extension.update((ZestStatement)parent.getZestElement(), action);
		}
	}
