 ...
 * Popup to allow the user to select all sites, or one site, and to display the Call Graph for their selection
 */
class PopupMenuCallGraph extends PopupMenuHttpMessageContainer {
	...
	private static final String POPUP_MENU_LABEL = Constant.messages.getString("callgraph.popup.option");
	private static final String POPUP_MENU_ALL_SITES = Constant.messages.getString("callgraph.popup.option.allsites");
	private static final String POPUP_MENU_ONE_SITE = Constant.messages.getString("callgraph.popup.option.onesite");
	...
	public PopupMenuCallGraph() {
		...
		CallGraphPopupMenuItem menuitemAllSites = new CallGraphPopupMenuItem(POPUP_MENU_ALL_SITES, NodeType.ALL_SITES);
	...
	private static class CallGraphPopupMenuItem extends PopupMenuItemHttpMessageContainer {
		...
		public boolean isButtonEnabledForSelectedHttpMessage(HttpMessage httpMessage) {
			if ( httpMessage == null ) {
				//if there was no message passed, the user can only select "ALL_SITES", and not "ONE_SITE"
				switch (nodeType) {
				case ALL_SITES:
					return true;
				case ONE_SITE:
		...
		public void performAction(HttpMessage httpMessage) {
			...
			case ONE_SITE:				
				...
				//parse out the scheme and authority, which is what we will use to filter requests for a single site.				
				try {
			...
			if (log.isDebugEnabled()) log.debug("Creating regular expression based on ^"+sitePattern +"$");
